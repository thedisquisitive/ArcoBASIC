...
DB_PATH="$REPO_ROOT/arcology-v01a.arcodb"
OUT_DIR="$REPO_ROOT/dist/arcology"
HOST="127.0.0.1"
PORT="8080"
MAX_REQUESTS="0"
BUILD_IF_MISSING=1
...
usage() {
    cat <<USAGE
Usage: scripts/serve-arcology.sh [options]

Options:
  --db PATH             Arcology database path (default: ./arcology-v01a.arcodb)
  --out DIR             Static export directory (default: ./dist/arcology)
...
Examples:
  ...
  scripts/serve-arcology.sh --port 8088
  scripts/serve-arcology.sh --host 0.0.0.0 --port 8080
USAGE
}
...
export ARCOBASIC_STDLIB="$REPO_ROOT/stdlib"
cd "$REPO_ROOT"

exec "$ARCOSH" --safe "$REPO_ROOT/examples/arcology_serve_static.abas" \
