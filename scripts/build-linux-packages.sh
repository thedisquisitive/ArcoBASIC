...
Usage:
  scripts/build-linux-packages.sh [--all] [--deb] [--rpm] [--tar] [--xbps-src] [--headless] [--full] [--out DIR]
...
Outputs:
  ...
  --xbps-src
           ...
           For release .xbps packages, use scripts/build-void-native-package.sh.
  --headless
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --xbps)
            ...
            echo "Use scripts/build-void-native-package.sh so Void packages are built natively through xbps-src." >&2
            exit 2
            ;;
        --tar|--tar.gz|--portable)
            want_tar=1
...
# Use scripts/build-void-native-package.sh to create a release .xbps package
