...
if [[ -f "$REPO_ROOT/CMakeLists.txt" ]]; then
    PACKAGE_SEARCH_DIR="$REPO_ROOT/dist"
else
    PACKAGE_SEARCH_DIR="$SCRIPT_DIR"
fi
...
Options:
  --latest    Use the newest *.deb in the local package directory. This is the default.
  --help      Show this help.
USAGE
}
...
latest_deb() {
    find "$PACKAGE_SEARCH_DIR" -maxdepth 1 -name '*.deb' -type f -printf '%T@ %p\n' 2>/dev/null | sort -nr | awk 'NR == 1 { print $2 }'
}
...
main() {
    ...
    if [[ -z "$deb" ]]; then
        echo "$SCRIPT_NAME: no Debian package found under: $PACKAGE_SEARCH_DIR" >&2
        exit 1
    fi
    if [[ ! -f "$deb" ]]; then
