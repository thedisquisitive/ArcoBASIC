...
# RFC-0053 Section 4: construction, indexing/property get+set, LEN, PRINT, nested
...
# Phase 2 (RFC-0053 Section 4): arrays and objects, always boxed through ArcoValue -- there is no
...
# Marshaling-loop audit (RFC-0053, "full Linux support" pass, the follow-up the project owner asked
...
# Classes and methods (Phase 2, RFC-0053 Section 4) -- a class instance IS an Object (with a
...
# Uncaught-error STDERR text (RFC-0053, "full Linux support" pass): previously only grepped for a
...
# Reference-counted lifetime tracking (RFC-0053, "full Linux support" pass): a Boxed local
...
# Tuples, string codepoint indexing, and range/bit-vector LEN+indexing (RFC-0053, "full Linux
# support" pass): three of the four remaining disclosed gaps from RFC-0053's own Phase 2 close-out
...
# RFC-0053, "full Linux support" pass. See .agents/ARCO_NATIVE_RUNTIME_PROGRESS.md's own
...
# Entry 23 (RFC-0053 Phase 14): the ACTUAL blocker on Arconaut's own native main loop, found by
...
# Entry 23 (RFC-0053 Phase 14): a local reassigned with a DIFFERENT physical representation across
...
# Entry 23 (RFC-0053 Phase 14): the RETURN-statement counterpart of the Store case just above --
...
# Entry 23 (RFC-0053 Phase 14): the untyped-parameter safety check's own necessary widening,
...
# Entry 24 (RFC-0053 Phase 15): a polymorphic instance-method dispatch site whose own override
...
# REVERTED during this same investigation (RFC-0053 Phase 15) after actually timing it: the raised
