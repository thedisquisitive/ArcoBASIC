...
# same detection scripts/run-uefi-hello.sh itself performs, checked again here so ctest reports a
# clean SKIP instead of the harness's own installation-guidance error text).
if ! command -v qemu-system-x86_64 > /dev/null 2>&1; then
    echo "SKIP: qemu-system-x86_64 is not installed; see scripts/run-uefi-hello.sh for installation guidance."
    exit 0
fi
if ! find /usr/share/ovmf /usr/share/OVMF -iname 'OVMF*.fd' 2>/dev/null | grep -v -i secboot | grep -q .; then
    echo "SKIP: no OVMF firmware image was found; see scripts/run-uefi-hello.sh for installation guidance."
    exit 0
fi

"$ARCOFISSION" build "$SOURCE_DIR/tests/systems/uefi-hello/hello.abas" -o "$TMP_ROOT/hello.efi" --target uefi-x86_64 > /dev/null
...
OUTPUT="$("$SOURCE_DIR/scripts/run-uefi-hello.sh" "$TMP_ROOT/hello.efi" "Hello from ArcoBASIC" 20)"
echo "$OUTPUT"
...
if "$SOURCE_DIR/scripts/run-uefi-hello.sh" "$TMP_ROOT/hello.efi" "This text never appears" 15 > "$TMP_ROOT/negative.out" 2>&1; then
