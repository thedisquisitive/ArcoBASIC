    ...
    for (const auto& current_block : target->blocks) {
        ...
        switch (instruction.kind) {
            ...
            case AmirInstruction::Kind::Load: {
                ...
                // this hit directly: a local reassigned across two branches with genuinely different
                // representations -- `appendStdout = FALSE` (a literal BOOL) in one, `appendStdout =
                // stdoutInfo.Append` (a Boxed object-field read) in the other -- is exactly the
                // "disagreement" shape infer_local_kind's own comment already documents and resolves
                // to Boxed (Kind::Store's codegen already boxes the literal FALSE via
                // arco_value_new_bool specifically so that answer holds no matter which branch
                // actually ran). But THIS load ignored that answer for anything other than Number and
                // fell back to the frontend's own per-branch "BOOL" hint, masking the freshly-boxed
                // pointer down to its low byte with the same AND 0xFF this comment's Number case
                // already describes -- confirmed by direct testing and reading the emitted assembly:
                // `STRING(appendStdout)` segfaulted dereferencing a 1-byte-truncated "pointer"
                // (0x00/0x01) as if it were the real ArcoValueBox. Fixed by widening this preference
                // to Boxed as well as Number -- both share the same "instruction.result_type cannot
                // be trusted over the real, disagreement-aware runtime kind" fix. (load_result_kind
                // itself is computed once, earlier in this case -- see the Number-cache comment near
                // the top -- and reused here unchanged.)
