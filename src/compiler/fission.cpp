...
private:
    ...
    std::string lower_call(AmirFunction& function, const CanonicalAstNode& node) {
        ...
        for (const auto& child : node.children) args.push_back(lower_expression(function, *child));
        // Default-parameter support (RFC-0053, "full Linux support" pass -- found genuinely
...
std::string declared_parameter_type(const std::string& declared_parameter) {
    ...
    // site actually needed lowering (RFC-0053, "full Linux support" pass): a TYPED parameter that
...
// the Array/Object/Index cases below and RFC-0053 Section 4, Phase 2). Never needs constructing
...
HostedValueKind infer_function_return_kind_uncached(const AmirModule& module, const AmirFunction& callee, int depth) {
    ...
    // Arconaut natively (RFC-0053 Phase 14) and confirmed with a minimal repro completely
...
// at runtime, not just hypothetically (RFC-0053 Phase 15): `arco::Value::to_string()` throwing
...
// Phase 2 classes (RFC-0053 Section 4): given a class name and a method name, walks
...
HostedValueKind infer_local_kind_uncached(const AmirModule& module, const AmirFunction& function,
    ...
    // DrawActions, RFC-0053 Phase 14): whichever branch runs at runtime decides which
    ...
    // Reference-counted lifetime tracking (RFC-0053, "full Linux support" pass): `local_name` might
...
HostedValueKind infer_hosted_value_kind_uncached(const AmirModule& module, const AmirFunction& function,
    ...
    // branching shape's own depth. Tried raising it to 64 (RFC-0053 Phase 15) specifically to
    ...
    for (const auto& block : function.blocks) {
        for (const auto& instruction : block.instructions) {
            ...
            if (instruction.kind == AmirInstruction::Kind::CallValue && instruction.result == name) {
                ...
                // Runtime.Args() (RFC-0053, "full Linux support" pass) always returns a real,
...
// kind's own collapsed answer for it is Boxed" (RFC-0053 Phase 14) from "this operand is
...
struct X86_64CodegenResult {
    ...
    // Linux support" pass found by actually running generated code (RFC-0053 Entries 20-21).
...
X86_64CodegenResult generate_x86_64_function(const AmirModule& module, const std::string& function_name,
    ...
    // Reference-counted lifetime tracking (RFC-0053, "full Linux support" pass): a PARAMETER's own
    ...
    const auto store_result = [&](const std::string& name, const std::string& type, Reg reg = Reg::RAX) -> bool {
        ...
        // Reference-counted lifetime tracking (RFC-0053, "full Linux support" pass): "STRING" is
    ...
    // rather than a bare type string -- SELF (Phase 2 classes, RFC-0053 Section 4) is a magic
    ...
    // Reference-counted lifetime tracking (RFC-0053, "full Linux support" pass): set by
    ...
    // Array/Object construction and StoreIndex's own value operand (Phase 2, RFC-0053 Section 4),
    ...
    for (const auto& current_block : target->blocks) {
        ...
        switch (instruction.kind) {
            ...
            // Same reasoning as DeclareFunction just above (Phase 2 classes, RFC-0053 Section 4):
            ...
            case AmirInstruction::Kind::Branch: {
                ...
                // two such comparisons (RFC-0053, "full Linux support" pass -- found genuinely
            ...
            case AmirInstruction::Kind::Load: {
                ...
                // until this real bug (RFC-0053, "full Linux support" pass -- found genuinely
                ...
                //
                // The identical hazard exists for Boxed, not just Number, and was missed here until
                // RFC-0052 WP-011 hit it directly: a local reassigned across two branches with
                // genuinely different representations -- `appendStdout = FALSE` (a literal BOOL) in
                // one, `appendStdout = stdoutInfo.Append` (a Boxed object-field read) in the other --
                // is exactly the "disagreement" shape infer_local_kind's own comment already
                // documents and resolves to Boxed (Kind::Store's codegen already boxes the literal
                // FALSE via arco_value_new_bool specifically so that answer holds no matter which
                // branch actually ran). But THIS load ignored that answer for anything other than
                // Number and fell back to the frontend's own per-branch "BOOL" hint, masking the
                // freshly-boxed pointer down to its low byte with the same AND 0xFF this comment's
                // Number case already describes -- confirmed by direct testing and reading the
                // emitted assembly: `STRING(appendStdout)` segfaulted dereferencing a 1-byte-truncated
                ...
                // Double-release bug (RFC-0053 native backend, found via the new --debug/--sanitize
            ...
            case AmirInstruction::Kind::Unary: {
                ...
                // "!" on a Boxed operand (an object field/array element/host-function-call result,
                // e.g. `!line.Ok`) needs the identical unboxing load_double_operand already applies
                // for Binary/Unary arithmetic just above -- confirmed missing by direct testing
                // (RFC-0053 Entry 25, Bug C): the fallthrough load_value/cmp-imm32 path below reads
                // the operand's raw ArcoValue* POINTER into RAX and masks/compares ITS bits
                // directly, as though it were already an unboxed 0/1 bool, instead of dereferencing
                // the box at all. `(pointer & 0xff) == 0` on a real heap pointer depends on the
                // allocator's own address, not the value stored inside, so this only ever "worked"
                // by heap-layout coincidence in the smallest repros and broke the instant a second
                // host call anywhere in the same loop shifted the allocation pattern (a stale,
                // wrong answer repeated forever from then on, not a crash -- the classic symptom of
                // a miscompilation rather than memory corruption; confirmed clean under
                // AddressSanitizer). See arco_value_truthy's own comment in native_runtime_abi.h.
                if (instruction.target == "!" &&
                    infer_hosted_value_kind(module, *target, instruction.operands.front()) == HostedValueKind::Boxed) {
                    if (!load_value(instruction.operands.front(), "STRING", Reg::RDI)) return result;
                    const auto call_disp = result.text.call_rel32_placeholder();
                    result.external_calls.push_back({call_disp, "arco_value_truthy"});
                    result.text.cmp_reg_imm32(Reg::RAX, 0);
                    result.text.setcc_al(0x4);
                    result.text.movzx_eax_al();
                    if (!store_result(instruction.result, instruction.result_type.empty() ? "BOOL" : instruction.result_type)) return result;
                    break;
                }
            ...
            case AmirInstruction::Kind::Binary: {
                ...
                // AND/OR/XOR where at least one side is a BOOL (RFC-0053, "full Linux support" pass
                ...
                // "==" / "!=" / "<" / "<=" / ">" / ">=" with an AMBIGUOUSLY Boxed or PROVABLY
                // String operand on either side -- the exact same "the ordinary hosted-number
                // branch just below treats Boxed as hosted-number-capable" hazard the "+" case
                // above this one exists for, just for comparison instead of addition. Found the
                // SAME way, immediately after fixing "+": `app.Mode == Lower(label)` (Arconaut's
                // own tab-highlighting check) -- BOTH sides Boxed (an object field, a generic
                // host-function result), so BOTH pass operand_is_hosted_number_for_binary's own
                // Boxed-is-hosted-number-capable rule and fell into the ordinary numeric-comparison
                // branch below (`ucomisd`/`sete`), which has no string awareness at all -- a clean
                // panic ("value is not a number") the moment either side turned out to actually be
                // the string it always is here, not a contrived edge case. The pre-existing
                // equality special case further down this function (`arco_value_equals`) already
                // exists for exactly this shape but is gated on `plausibly_string` (PROVABLY String
                // on at least one side, e.g. `row.Status == "active"`) -- never reached when
                // NEITHER side is provably String, only ambiguously Boxed, since the numeric branch
                // below claims the instruction first. Moved earlier and widened here to catch that
                // case too; the later block is consequently unreachable for "==" / "!=" now (every
                // Boxed/String combination is caught here first, and a case with neither is
                // correctly still real numeric comparison below) but left in place as a harmless,
                // already-correct fallback.
                //
                // "<"/"<="/">"/">=" widened in alongside "=="/"!=" here (RFC-0052 WP-006 --
                // ArcoSH's own `ch >= "0" ANDALSO ch <= "9"`-shaped digit check, both operands
                // PROVABLY String, is exactly this shape and had NO codegen anywhere for a
                // relational operator on a string: this whole file's ordinary numeric-comparison
                // path below unconditionally calls `.as_number()`-equivalent unboxing, and this is
                // a language-level gap, not something specific to hosted/Boxed values -- the
                // interpreter's own `apply_binary` panicked identically before this pass, see
                // arco::values_compare's own comment in include/arco/value.hpp). Ordering has TWO
                // possible comparand shapes (lexicographic string, numeric) depending on the
                // runtime values, unlike equality's single BOOL-shaped answer, so it needs its own
                // ABI call (`arco_value_compare`, mirrors arco::values_compare exactly: returns
                // -1/0/1) rather than reusing `arco_value_equals`; the result is turned into a BOOL
                // here with an ordinary signed `cmp`+`setcc` against zero, one condition code per
                // operator, the same pattern the numeric ucomisd path below already uses for its
                // own four-operator case.
                if (convention == systems::CallingConvention::SystemV &&
                    ...
                    if (instruction.target == "==" || instruction.target == "!=") {
                        ...
                        {
                            const auto call_disp = result.text.call_rel32_placeholder();
                            result.external_calls.push_back({call_disp, "arco_value_compare"});
                        }
                        // arco_value_compare returns a 32-bit `int` (-1/0/1) in EAX; the System V
                        // ABI leaves the upper 32 bits of RAX UNSPECIFIED for a 32-bit return value
                        // (never guaranteed sign-extended by the callee), so a 64-bit `cmp rax, 0`
                        // against the -1 case is comparing garbage-in-the-high-bits, not really -1
                        // -- confirmed directly: `"abc" < "abd"` (a true "<0" result) came back
                        // FALSE without this. Sign-extend EAX into RAX first (shl 32 / sar 32, a
                        // CDQE-equivalent this encoder has no dedicated opcode for) so the 64-bit
                        // cmp below sees the real signed value.
                        result.text.shl_reg_imm8(Reg::RAX, 32);
                        result.text.sar_reg_imm8(Reg::RAX, 32);
                        result.text.cmp_reg_imm32(Reg::RAX, 0);
                        std::uint8_t condition = 0xC; // JL/SETL -- "<"
                        if (instruction.target == "<=") condition = 0xE; // SETLE
                        else if (instruction.target == ">") condition = 0xF; // SETG
                        else if (instruction.target == ">=") condition = 0xD; // SETGE
                ...
                if (left_is_hosted_number && right_is_hosted_number) {
                    ...
                    // loop audit this comment's own bug-history entry above prompted (RFC-0053,
                ...
                if (convention == systems::CallingConvention::SystemV) {
                    ...
                    if (instruction.target == "+" && (plausibly_string(left_kind) || plausibly_string(right_kind))) {
                        ...
                        // every concatenation of a fresh scalar (RFC-0053, "full Linux support" pass
                    ...
                    // General equality (RFC-0053, "full Linux support" pass -- found genuinely
            ...
            case AmirInstruction::Kind::Store: {
                ...
                // non-Boxed source" no longer holds unconditionally (RFC-0053, "full Linux support"
            ...
            case AmirInstruction::Kind::CallValue: {
                ...
                const auto call_resolved_method = [&](const AmirFunction& resolved,
                    ...
                    // force_boxed (RFC-0053 Phase 15): set by a polymorphic dispatch site (instance
                ...
                // that never references Args itself, on every target. Real support (RFC-0053, "full
                ...
                //
                // MUST also be gated on convention == MicrosoftX64: found missing by direct testing
                // (RFC-0053, ArcoSH WP-003 tokenizer) -- lower_call's "STRING" operand_types hint
                // for LEN/MID is set whenever the argument is typed STRING at all, regardless of
                // which convention is being compiled for, so an ordinary hosted `FUNCTION
                // f(text AS STRING) ... LEN(text)` under System V fell into this freestanding-only
                // "raw UTF-16 buffer" walk too -- treating a real Boxed ArcoValue* pointer as if it
                // were a null-terminated UTF-16 buffer, walking off into unrelated heap bytes until
                // one happened to look like a zero unit, and silently storing whatever small
                // (wrong) count that produced as a real U64 length. No crash, no ASan hit (reading
                // heap memory that belongs to the process, just not to a real string) -- the loop
                // bound was just quietly wrong, terminating a WHILE loop after 1 iteration instead
                // of running the real string's full length. The correct System V path (below, gated
                // on infer_hosted_value_kind == Boxed/String, calling the real arco_value_length)
                // already existed and was simply unreachable for this shape until now.
                const std::string upper_call_target = upper_ascii(instruction.target);
                const bool string_len_or_mid = convention == systems::CallingConvention::MicrosoftX64 &&
                    (upper_call_target == "LEN" || upper_call_target == "MID") &&
                ...
                // Instance method dispatch (Phase 2 classes, RFC-0053 Section 4):
                ...
                if (!callee && convention == systems::CallingConvention::SystemV) {
                    ...
                    if (last_dot != std::string::npos && slot_of(base_name) >= 0) {
                        ...
                        if (!candidates.empty()) {
                            ...
                            // RFC-0053 Phase 15: whether THIS call site's own override candidates
                            ...
                            for (const auto& [class_name, resolved_name] : candidates) {
                                ...
                                // this fix (RFC-0053, "full Linux support" pass -- found by direct
                ...
                // ADDRESSOF/CALLABLE dispatch (RFC-0053): `f(...)` where `f` is a plain local
                ...
                if (!callee && convention == systems::CallingConvention::SystemV && slot_of(instruction.target) >= 0) {
                    ...
                    }
                    {
                        ...
                        // RFC-0053 Phase 15: same as instance dispatch's own identical computation
                ...
                // Generic host-function bridge fallback (RFC-0053, "finish off Linux support"):
                ...
                // unattempted future work; see RFC-0053 and .agents/ARCO_NATIVE_RUNTIME_PROGRESS.md.
                if (convention == systems::CallingConvention::SystemV) {
                    ...
                    // own identical two-pass comment (RFC-0053, "full Linux support" pass) for the
                    ...
                    for (std::size_t i = 0; i < instruction.operands.size(); ++i) {
                        ...
                        if (param_is_hosted_number(callee->params[i])) {
                            ...
                            // work -- see RFC-0053. An EXPLICITLY "AS NUMBER"-typed parameter skips
                            ...
                            if (param_type.empty()) {
                                ...
                                // Boxed value could genuinely be a number (RFC-0053 Phase 14,
            ...
            case AmirInstruction::Kind::Return: {
                ...
                if (value_ref == "nothing" && convention == systems::CallingConvention::SystemV) {
                    ...
                    // Multi-RETURN ambiguity (RFC-0053 Phase 14, found by actually running Arconaut
                ...
                // Reference-counted lifetime tracking (RFC-0053, "full Linux support" pass): every
            ...
            // Phase 2 (RFC-0053 Section 4): arrays and objects, both always boxed through
            ...
            // Tuples (RFC-0053, "full Linux support" pass): every element is available up front
            ...
            case AmirInstruction::Kind::Index: {
                ...
                // indexing (RFC-0053, "full Linux support" pass): boxed fresh just to satisfy
            ...
            // ADDRESSOF (RFC-0053): produces a CALLABLE value for `instruction.target` (a bare
            ...
            // TRY/CATCH (RFC-0053): generated code has no unwind tables, so this calls the real
...
Value eval_binary(const std::string& op, const Value& left, const Value& right) {
    ...
    if (op == "<") {
        return values_compare(left, right) < 0;
    }
    if (op == "<=") {
        return values_compare(left, right) <= 0;
    }
...
Result build_linux_native_image(const std::string& source, const std::string& source_name,
    ...
    try {
        ...
        // repro (RFC-0053 Entry 21) -- previously done by hand-patching this exact args list,
