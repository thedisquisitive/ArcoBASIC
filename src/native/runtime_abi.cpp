...
int arco_value_compare(const ArcoValue* left, const ArcoValue* right) {
    const arco::Value left_value = left == nullptr ? arco::Value() : box(left)->value;
    const arco::Value right_value = right == nullptr ? arco::Value() : box(right)->value;
    // arco::values_compare throws (via Value::as_number()) for a non-orderable, non-string value --
    // converted to a panic here, same discipline as arco_value_as_number just below (see
    // native_runtime_abi.h's own "no C++ exceptions cross this ABI boundary" rule).
    try {
        return arco::values_compare(left_value, right_value);
...
int arco_value_truthy(const ArcoValue* value) {
    // arco::Value::truthy() treats a null Value() (not a null ArcoValue* pointer -- see is_null())
    // as falsy already; a genuinely null POINTER (this backend's own "nothing" sentinel, e.g. an
    // unset typed class field -- see arco_value_object_set's own comment) is handled the same way
    // here for consistency, never dereferenced.
    if (value == nullptr) return 0;
    return box(value)->value.truthy() ? 1 : 0;
}
...
void arco_value_print(const ArcoValue* value) {
    ...
    // Flushed explicitly (RFC-0053, "full Linux support" pass): stdout is fully buffered whenever
    ...
    // (RFC-0053 Phase 11's own follow-up pass): every PRINT trace this file's own generated code
...
void arco_value_panic(const char* message) {
    ...
    // (RFC-0053, "full Linux support"): the two previously differed ("runtime error: <message>"
