...
* Core runtime optional libcurl-backed networking helpers available to interpreter and ArcoFission-hosted code: `Network.Available`, `Network.Get`, `Network.Post`, `Network.Download`
...
* File, Directory, and Path helpers for scripting: write/append text, create/check directories, and common path transforms
...
* Single-line `IF condition THEN statement`, `==` equality, and `Exit()`/`ExitTheProgram()` helpers
...
* Named stdlib import resolution for `#IMPORT "text"`, `#IMPORT "files"`, `#IMPORT "compy"`, `#IMPORT "compydb"`, and `#IMPORT "arcodb"` via local paths, `stdlib/`, `../stdlib/`, installed share directories, and `ARCOBASIC_STDLIB`
...
* Classic `STOP` support to end the current ArcoBASIC program
* Runtime tests
...
* An interactive shell (ArcoSH is being rebuilt from scratch; see git history for the previous implementation)
