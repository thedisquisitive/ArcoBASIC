...
Depends on: `docs/systems/x86-64-codegen.md`, `docs/systems/uefi-target.md` section 9 (strategy
decision)

`include/arco/pe_image.hpp` / `compiler/pe_image.cpp` write a minimal PE32+ EFI application
...
*next* instruction). Verified both by a unit test (`tests/runtime_tests.cpp`) computing the exact
...
construction — see `docs/systems/x86-64-codegen.md`), so the image behaves correctly no matter
