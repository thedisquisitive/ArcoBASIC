...
Depends on: `docs/systems/pe32-image.md`

`scripts/run-uefi-hello.sh` is a reusable, deterministic harness that boots a built ArcoBASIC UEFI
application under real QEMU + OVMF firmware and checks its console output for an expected string.
`tests/systems_qemu_ovmf_harness_smoke.sh` wires it into the automated test suite for the
hello-world program specifically.

This formalizes the ad hoc boot check performed manually while implementing WP-009 (see
`docs/systems/pe32-image.md`) into the reusable tool Packet WP-010 asks for, usable both by the
test suite and directly by a developer.

## Usage

```sh
scripts/run-uefi-hello.sh EFI_FILE EXPECTED_OUTPUT [TIMEOUT_SECONDS]
```
...
`scripts/run-uefi-hello.sh` checks for `qemu-system-x86_64` on `PATH` and an `OVMF*.fd` file under
...
`tests/systems_qemu_ovmf_harness_smoke.sh` performs the same detection itself before calling the
...
`tests/systems_qemu_ovmf_harness_smoke.sh` additionally proves the harness is not vacuously
