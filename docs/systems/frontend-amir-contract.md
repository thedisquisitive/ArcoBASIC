...
`CanonicalAstNode` in `core/parser.hpp` is the compiler-facing representation. It records a stable
...
Every accepted `Expr` and `Stmt` supplies `canonical_ast()`. `compiler/fission.cpp` accepts the
...
`tests/systems_frontend_amir_contract_smoke.sh` covers representative expressions, statements,
