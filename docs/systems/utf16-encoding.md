...
Depends on: `docs/systems/uefi-bindings.md` (confirms `EFI_TEXT_STRING`'s `String` parameter is
`CHAR16*`)
...
occurred; `Parser::validate_utf16_arguments` (`core/parser.cpp`) wraps this into a source-location
...
`tests/systems_utf16_encoding_smoke.sh`.
