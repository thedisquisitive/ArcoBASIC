...
Depends on: `arcology-os/docs/systems/calling-conventions.md`, `arcology-os/docs/systems/uefi-bindings.md`,
...
`arcology-os/docs/systems/calling-conventions.md` documents (`RSP % 16 == 8` at function entry, so
...
`arcology-os/docs/systems/uefi-bindings.md` flagged that `EFI_SIMPLE_TEXT_OUTPUT_PROTOCOL.OutputString`'s real
...
`tests/unit/runtime_tests.cpp` against byte sequences independently produced by `nasm -f bin` (not
...
`arcology-os/tests/systems/systems_x86_64_codegen_smoke.sh` golden-tests the full, exact hex dump of
...
`arcology-os/docs/systems/uefi-target.md` section 10's "no new dependency" decision; nasm/objdump were already
...
- Does not produce a PE32+ file, ELF, or any other container format -- `render_x86_64` emits a
  ...
  `src/compiler/fission.cpp` until WP-009 needs them.
