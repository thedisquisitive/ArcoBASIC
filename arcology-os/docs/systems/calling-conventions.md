...
Depends on: `arcology-os/docs/systems/uefi-target.md` section 4
...
registers) because no floating-point ABI work is in scope (`arcology-os/docs/systems/uefi-target.md`,
...
convention needed for this milestone (`arcology-os/docs/systems/uefi-target.md` section 5: the UEFI entry
...
expose these lists (unit-tested to be disjoint in `tests/unit/runtime_tests.cpp`). No register
...
CallExternal`, arcology-os/docs/systems/uefi-target.md via `.agents/reports/WP-004-amir-systems-primitives.md`)
...
`arcology-os/tests/fixtures/uefi-hello/hello.abas`:
