...
Status: draft (WP-001)
Mission: `arcology-os/agent-packets/Arcology_ArcoBASIC_Systems_Agent_Packet.md`
...
The shared preprocessor (`Runtime::preprocess_source`, `src/runtime/runtime.cpp`)
...
not duplicated in `src/compiler/fission.cpp`.
...
variable declaration is `LET name = expr` (`src/frontend/parser.cpp`,
...
(`Parser::parse_type_name`, `src/frontend/parser.cpp:1532`) but not for `LET`.
...
identifier-continuation character (`src/frontend/lexer.cpp:251`), so `UEFI.Handle`
...
`arcology-os/examples/uefi_hello.abas` (see also `arcology-os/tests/fixtures/uefi-hello/hello.abas`,
...
`arcology-os/tests/fixtures/uefi-hello/expected-output.txt` holds exactly this line
...
`apps/arcofission/main.cpp`; it does not change the meaning of `build`
...
- New UEFI bindings registry and compile-time field-chain validation
  ...
  `Parser::validate_uefi_field_chain`. See `arcology-os/docs/systems/uefi-bindings.md`.
...
- New PE32+ EFI image writer (WP-009): `include/arco/pe_image.hpp` /
  `src/compiler/pe_image.cpp`, `ArcoFission build FILE -o OUT.efi --target
  uefi-x86_64`. The built hello-world image boots under real QEMU/OVMF
  and prints `Hello from ArcoBASIC`. See `arcology-os/docs/systems/pe32-image.md`.
- New reusable QEMU/OVMF boot harness (WP-010): `arcology-os/scripts/run/run-uefi-hello.sh`.
  See `arcology-os/docs/systems/qemu-ovmf-harness.md`.
