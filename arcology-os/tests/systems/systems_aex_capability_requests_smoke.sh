...
ARCOFISSION="$1"
SOURCE_DIR="$2"
TMP_ROOT="${TMPDIR:-/tmp}/aex-capability-requests-smoke-$$"
mkdir -p "$TMP_ROOT"
trap 'rm -rf "$TMP_ROOT"' EXIT

# RFC-0046 AEX Phase 9: capability request evaluation. The composed file includes ArcFS's active
...
COMPOSED="$TMP_ROOT/aex-capability-requests.abas"
{
    printf '#PROFILE UEFI\n#TARGET X86_64\n#RUNTIME NONE\n\n'
    ...
    printf '\n'
    sed '/^#PROFILE /d;/^#TARGET /d;/^#RUNTIME /d' "$SOURCE_DIR/arcology-os/tests/fixtures/aex-capability-requests/aex-capability-requests-body.abas"
} > "$COMPOSED"

"$ARCOFISSION" reveal "$COMPOSED" at X86_64 --entry Main > "$TMP_ROOT/x86.txt" 2>&1
grep -qF 'X86_64 GENERATED' "$TMP_ROOT/x86.txt" || {
    echo "FAIL: aex-capability-requests does not compile:" >&2
...
for CASE in "OPTN : PASS" "REQD : PASS" "GRNT : PASS" "WRNG : PASS" "BNDS : PASS" "OWNR : PASS" "REQT : PASS" "IDMM : PASS" "RSVD : PASS"; do
...
echo "PASS: RFC-0046 AEX Phase 9 -- capability request metadata and deterministic required-denial evaluation confirmed under QEMU"
