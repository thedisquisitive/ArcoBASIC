...
ARCOFISSION="$1"
SOURCE_DIR="$2"
TMP_ROOT="${TMPDIR:-/tmp}/aex-native-lifecycle-smoke-$$"
mkdir -p "$TMP_ROOT"
trap 'rm -rf "$TMP_ROOT"' EXIT

# RFC-0046 AEX Phase 7: native ABI lifecycle invocation. The composed file includes the same
# ArcFS slice as Phase 6 because the active AEX policy still contains AEX.LoadFromArcFS and the
# current systems backend emits declared helpers eagerly.

COMPOSED="$TMP_ROOT/aex-native-lifecycle.abas"
{
    printf '#PROFILE UEFI\n#TARGET X86_64\n#RUNTIME NONE\n\n'
    ...
    printf '\n'
    sed '/^#PROFILE /d;/^#TARGET /d;/^#RUNTIME /d' "$SOURCE_DIR/arcology-os/tests/fixtures/aex-native-lifecycle/aex-native-lifecycle-body.abas"
} > "$COMPOSED"

"$ARCOFISSION" reveal "$COMPOSED" at X86_64 --entry Main > "$TMP_ROOT/x86.txt" 2>&1
grep -qF 'X86_64 GENERATED' "$TMP_ROOT/x86.txt" || {
    echo "FAIL: aex-native-lifecycle does not compile:" >&2
...
for CASE in "LIFE : PASS" "CALL : PASS" "DEAC : PASS" "OFFS : PASS" "MISS : PASS" "OWNR : PASS"; do
...
echo "PASS: RFC-0046 AEX Phase 7 -- lifecycle metadata resolution and no-argument native ABI activation confirmed under QEMU"
