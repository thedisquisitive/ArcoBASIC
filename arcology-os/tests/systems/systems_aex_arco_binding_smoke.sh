...
ARCOFISSION="$1"
SOURCE_DIR="$2"
TMP_ROOT="${TMPDIR:-/tmp}/aex-arco-binding-smoke-$$"
mkdir -p "$TMP_ROOT"
trap 'rm -rf "$TMP_ROOT"' EXIT

# RFC-0046 AEX Phase 10: ArcoBASIC Binding Table registration. The composed file includes ArcFS's
...
COMPOSED="$TMP_ROOT/aex-arco-binding.abas"
{
    printf '#PROFILE UEFI\n#TARGET X86_64\n#RUNTIME NONE\n\n'
    ...
    printf '\n'
    sed '/^#PROFILE /d;/^#TARGET /d;/^#RUNTIME /d' "$SOURCE_DIR/arcology-os/tests/fixtures/aex-arco-binding/aex-arco-binding-body.abas"
} > "$COMPOSED"

"$ARCOFISSION" reveal "$COMPOSED" at X86_64 --entry Main > "$TMP_ROOT/x86.txt" 2>&1
grep -qF 'X86_64 GENERATED' "$TMP_ROOT/x86.txt" || {
    echo "FAIL: aex-arco-binding does not compile:" >&2
...
for CASE in "REGS : PASS" "ADEN : PASS" "AGRN : PASS" "BNDS : PASS" "OWNR : PASS" "KIND : PASS" "IDMM : PASS" "TARG : PASS" "TOWN : PASS" "RSVD : PASS"; do
...
echo "PASS: RFC-0046 AEX Phase 10 -- ArcoBASIC Binding Table metadata registration confirmed under QEMU"
