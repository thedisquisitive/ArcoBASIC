...
ARCOFISSION="$1"
SOURCE_DIR="$2"
TMP_ROOT="${TMPDIR:-/tmp}/aex-negative-matrix-smoke-$$"
mkdir -p "$TMP_ROOT"
trap 'rm -rf "$TMP_ROOT"' EXIT

# RFC-0046 AEX Phase 13: deterministic malformed-input matrix over the implemented loader
# surfaces. The composed file includes ArcFS's active in-memory object/handle slice because the
# active AEX policy still contains AEX.LoadFromArcFS and the current systems backend emits declared
# helpers eagerly.

COMPOSED="$TMP_ROOT/aex-negative-matrix.abas"
{
    printf '#PROFILE UEFI\n#TARGET X86_64\n#RUNTIME NONE\n\n'
    ...
    printf '\n'
    sed '/^#PROFILE /d;/^#TARGET /d;/^#RUNTIME /d' "$SOURCE_DIR/arcology-os/tests/fixtures/aex-negative-matrix/aex-negative-matrix-body.abas"
} > "$COMPOSED"

"$ARCOFISSION" reveal "$COMPOSED" at X86_64 --entry Main > "$TMP_ROOT/x86.txt" 2>&1
grep -qF 'X86_64 GENERATED' "$TMP_ROOT/x86.txt" || {
    echo "FAIL: aex-negative-matrix does not compile:" >&2
...
for CASE in "VALD : PASS" "MAGC : PASS" "HDRS : PASS" "DIRB : PASS" "UNKN : PASS" "CHNK : PASS" "MANF : PASS" "IMPL : PASS" "INTG : PASS" "BNDG : PASS" "DIAG : PASS"; do
...
echo "PASS: RFC-0046 AEX Phase 13 -- deterministic malformed-input matrix confirmed under QEMU"
