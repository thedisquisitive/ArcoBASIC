...
# RFC-0012 forbids the historical second parser in src/compiler/fission.cpp. These source checks are
...
if grep -q 'build_amir(tokens' "$SOURCE_DIR/src/compiler/fission.cpp"; then
    echo "FAIL: Fission still builds A-MIR directly from lexer tokens" >&2
    exit 1
fi
if grep -q '^class AmirBuilder' "$SOURCE_DIR/src/compiler/fission.cpp"; then
    echo "FAIL: the historical token-reinterpreting AmirBuilder still exists" >&2
    exit 1
fi
if grep -q '(void)parser.parse' "$SOURCE_DIR/src/compiler/fission.cpp"; then
