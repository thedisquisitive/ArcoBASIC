...
ARCOFISSION="$1"
SOURCE_DIR="$2"
TMP_ROOT="${TMPDIR:-/tmp}/aex-integrity-verification-smoke-$$"
mkdir -p "$TMP_ROOT"
trap 'rm -rf "$TMP_ROOT"' EXIT

# RFC-0046 AEX Phase 11: integrity verification. The composed file includes ArcFS's active
...
COMPOSED="$TMP_ROOT/aex-integrity-verification.abas"
{
    printf '#PROFILE UEFI\n#TARGET X86_64\n#RUNTIME NONE\n\n'
    ...
    printf '\n'
    sed '/^#PROFILE /d;/^#TARGET /d;/^#RUNTIME /d' "$SOURCE_DIR/arcology-os/tests/fixtures/aex-integrity-verification/aex-integrity-verification-body.abas"
} > "$COMPOSED"

"$ARCOFISSION" reveal "$COMPOSED" at X86_64 --entry Main > "$TMP_ROOT/x86.txt" 2>&1
grep -qF 'X86_64 GENERATED' "$TMP_ROOT/x86.txt" || {
    echo "FAIL: aex-integrity-verification does not compile:" >&2
...
for CASE in "INTG : PASS" "TAMP : PASS" "MISS : PASS" "BNDS : PASS" "ALGO : PASS" "COVR : PASS" "LENG : PASS"; do
...
echo "PASS: RFC-0046 AEX Phase 11 -- protected CODE integrity metadata and mutation rejection confirmed under QEMU"
