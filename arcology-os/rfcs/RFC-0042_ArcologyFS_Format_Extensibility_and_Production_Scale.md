...
**Status:** Draft (All five named phases -- N, O, P, Q, R -- implemented and QEMU-proven. Phases N and O, delivered together: Section 7's real feature negotiation for the superblock's own long-unused flags field, Section 8's self-describing checkpoint record (an append-only `recordLength` field disambiguated from every pre-RFC-0042 checkpoint by checksum validity, not a version flag -- moving it to offset 0 was considered and rejected during drafting since it would have broken every existing FMV2 checkpoint), and Section 9's real backward-compatibility proof against an actually-downgraded on-disk checkpoint shape. Two real bugs found and fixed along the way: `ArcFSReadCheckpoint`'s own `attributeTreeCount` bound checked the wrong constant (see `.agents/reports/aps-arcfs-attribute-bound-fix.md`), and `ArcFS.MountImage` never cleared `ArcFSReadOnlySafetyAddress` itself, leaving a ro-compat-triggered read-only condition stuck across later, genuinely clean remounts -- see `.agents/reports/aps-arcfs-phase-n-o.md`. Phase P (reserved row capacity) -- the Object (40->56 bytes), Namespace (64->80 bytes), and Attribute (56->64 bytes) live rows each gained genuinely unpopulated reserved fields for future per-row growth, with the on-disk tree leaf formats left completely unchanged (nothing new to persist yet); every widened table fit its existing fixed address headroom with no relocation needed, confirmed directly -- see `.agents/reports/aps-arcfs-phase-p.md`. Phase Q (production-scale capacities) -- ArcFSMaxObjects() 64->2048, ArcFSMaxNamespaceRows()/ArcFSMaxAttributes() 128->4096, ArcFSMaxHandles() 8->64, ArcFSMaxFileChunks() 4->8 (32 KiB max files), all target numbers revised down from this RFC's own original draft after tracing the Data Pool's real multiplicative footprint through concretely (the original 4096-object/1MiB-file table together would have needed a 4 GiB Data Pool); required first giving the QEMU test harness explicit, empirically-verified RAM (was silently defaulting to 128 MiB, now 512 MiB -- see `.agents/reports/aps-qemu-ram-ceiling.md`) and then a full address-map replanning for every capacity-scaling structure; found and fixed 3 more stale hardcoded capacity-derived literals beyond the one already fixed in a prior increment. A real, honestly-named coupling surfaced while designing the proof: the Allocation Bitmap's own volume-size ceiling (~254 KB, unchanged this phase) is tightly bound to object count since every commit bulk-rebuilds the whole Object+Namespace trees, so the new 2048-object ceiling is a real, proven LIVE capacity today but not yet a real, proven PERSISTED one at anywhere near that scale -- see `.agents/reports/aps-arcfs-phase-q.md`. Phase R (UTF-8/blob attribute values) -- new `ArcFSAttrTypeString()`/`ArcFSAttrTypeBlob()` type codes plus `ArcFS.SetStringAttribute`/`GetStringAttribute`/`SetBlobAttribute`/`GetBlobAttribute`, reusing the existing Data Pool + Extent Tree machinery (`valueLow`/`valueHigh` change MEANING for these two type codes, not width -- zero on-disk Attribute Tree format change); a real design flaw (attribute-row-index cannot safely reuse the file-row's own row-index-as-live-slot-identity trick, since the two domains are different sizes since Phase Q) was caught and fixed before any QEMU run, along with a real mount-ordering bug it exposed (`ArcFS.MountImage`'s data-slot bump-counter update needed to move one call earlier) -- see `.agents/reports/aps-arcfs-phase-r.md`. The Allocation Bitmap's own multi-sector redesign, named by Phase Q's own coupling finding, remains real, honestly-scoped future work beyond this RFC's five named phase letters.)\
...
**Revised during implementation from this section's own original draft, honestly, per Section
15.2's own requirement.** The original table below raised `ArcFSMaxObjects()` and
`ArcFSMaxFileChunks()` independently to 4096 and 256 -- traced through concretely once
implementation began: the Data Pool's real size is the PRODUCT
`ArcFSMaxObjects() * ArcFSMaxFileChunks() * ArcFSFileCapacityBytes()`, not a sum, and at those two
numbers together that product is 4 GiB -- wildly outside any real RAM budget, even the harness's
own revised 512 MiB (Section 11.2). The delivered targets instead raise object/namespace/attribute
row COUNTS (cheap, additive) by the full original multiplier, while raising file-chunk count
(expensive, multiplicative with object count) by far less, keeping the Data Pool at a real, fully
backed 64 MiB:

| Constant                        | RFC-0040 value | Originally drafted | Actually delivered |
|----------------------------------|----------------|---------------------|----------------------|
| `ArcFSMaxObjects()`              | 64             | 4096                | 2048                 |
| `ArcFSMaxNamespaceRows()`        | 128            | 8192                | 4096                 |
| `ArcFSMaxAttributes()`           | 128            | 8192                | 4096                 |
| `ArcFSMaxHandles()`              | 8              | 64                  | 64                   |
| `ArcFSMaxTrackedSectors()`       | 508 (~254 KB)  | 131064 (~64 MB)     | 508 (~254 KB, unchanged) |
| `ArcFSMaxFileChunks()`           | 4 (16 KB)      | 256 (1 MB)          | 8 (32 KB)            |
...
small tree or a fixed run of self-checksummed sectors, whichever proves simpler) -- named as this
section's own largest single piece of work when this RFC was first drafted, confirmed as exactly
that once implementation reached it: a real, important, and previously-uncharacterized coupling
surfaced during that work (every commit bulk-rebuilds the whole Object+Namespace trees fresh, so
real on-disk cost per object is dominated by tree-node overhead, not file content -- meaning the
volume-size ceiling and the object-count ceiling are NOT independent the way this table's own
original layout implied). `ArcFSMaxObjects()=2048` is delivered as a real, proven LIVE (in-memory,
single-session) capacity; genuinely persisting anywhere near that many objects still needs this
row's own remaining work. See `.agents/reports/aps-arcfs-phase-q.md` for the complete analysis.
...
for the String type; `ArcFS.SetBlobAttribute`/`ArcFS.GetBlobAttribute` give the Blob type its own
symmetric pair (implemented as thin wrappers over the exact same shared internal mechanism, since
Section 12.1 already treats both as the same representation with only the type code differing) so
callers never need to overload the String API for binary content. `ArcFS.RemoveAttribute` (already
generic over `attributeId`) needs no change.
...
| 0.3     | 2026-08-21 | Phase P (Section 10, reserved growth capacity) implemented and QEMU-proven. All three live row layouts widened -- Object 40->56 bytes (two reserved U64 fields), Namespace 64->80 bytes (two reserved U64 fields), Attribute 56->64 bytes (one reserved U64 field) -- with the ON-DISK tree leaf entry shapes for all three trees left completely unchanged, since nothing populates the new reserved fields yet; this reduced the whole increment to three stride constants plus `ArcFS.Initialize()`'s three clear-loop sizes, no per-row-constructor changes needed (the "always zero" property falls out of the existing full-table clear plus no function ever writing past its own already-defined fields). Every widened table's address headroom confirmed directly by reading the real address constants, not estimated -- all three fit their existing fixed MMIO gaps with no relocation. Real QEMU/OVMF proof (`aps-arcfs-reserved-rows.abas`): two objects with real namespace entries and real attributes -- every new reserved field reads exactly zero immediately after creation, after a real commit+remount, and after a real rename and reflink followed by another commit+remount, with every existing structural check unaffected. Passed first attempt; negative control confirmed real `FAIL 1`; deterministic across 3 runs. New smoke test `systems_arco_basic_arcfs_reserved_rows_smoke`; full suite re-run clean (81/81). See `.agents/reports/aps-arcfs-phase-p.md`. |
| 0.4     | 2026-08-21 | Phase Q (Section 11, production-scale capacities) implemented and QEMU-proven. Required a prerequisite fix first: the QEMU test harness never passed an explicit `-m` flag, silently defaulting to 128 MiB (confirmed empirically via a probe fixture, not assumed) -- every harness script now passes `-m 512`, re-confirmed empirically (see `.agents/reports/aps-qemu-ram-ceiling.md`, committed separately as test infrastructure). Section 11.1's own target table revised down honestly from its original draft: the Data Pool's real size is the PRODUCT of `ArcFSMaxObjects()` and `ArcFSMaxFileChunks()`, not a sum, and the original 4096/256 pairing would have needed a 4 GiB Data Pool. Delivered: `ArcFSMaxObjects()` 64->2048, `ArcFSMaxNamespaceRows()`/`ArcFSMaxAttributes()` 128->4096, `ArcFSMaxHandles()` 8->64, `ArcFSMaxFileChunks()` 4->8 (32 KiB max files) -- `ArcFSMaxTrackedSectors()` (the Allocation Bitmap's own volume-size ceiling) stays unchanged at 508 (~254 KB), its own multi-sector redesign left as this RFC's own remaining, explicitly-named future work. Every capacity-scaling structure relocated to a fresh, generously-spaced 0x10000000+ region (18 MiB of small structure blocks plus the Data Pool's own dedicated 128 MiB block, ending at 416 MiB, comfortably inside the new 512 MiB harness budget) -- confirmed via a direct, complete address-and-range collision check across the whole file. Found and fixed 3 more stale hardcoded capacity-derived literals (`256`) beyond the one already fixed in a prior increment, all now re-derived from the named constants. A real, previously-uncharacterized coupling surfaced while designing the proof fixture: every commit bulk-rebuilds the whole Object+Namespace trees fresh, so real on-disk cost per object is dominated by tree-node overhead, not file content -- meaning the volume-size and object-count ceilings are NOT independent, and persisting anywhere near the new 2048-object LIVE ceiling needs roughly 800 KB+ of tree nodes alone, several times the current 254 KB volume ceiling. Real QEMU/OVMF proof (`aps-arcfs-production-scale.abas`), two scenarios on two separate volumes for exactly that reason: 80 real objects (25% more than the old 64-object ceiling, fitting the current volume ceiling) forcing a genuinely deep 4-level Object Tree, real commit+remount, real path resolution for the first/middle/last object; and, on a separate freshly-formatted volume, one file using the full new 32 KiB max file size, content verified byte-for-byte across all 32768 bytes after a real commit+remount. Passed first attempt; negative control confirmed real `FAIL 1`; deterministic across 3 runs. New smoke test `systems_arco_basic_arcfs_production_scale_smoke`; full suite re-run clean (82/82). See `.agents/reports/aps-arcfs-phase-q.md`. |
| 0.5     | 2026-08-21 | Phase R (Section 12, UTF-8 string / binary blob attribute values) implemented and QEMU-proven -- the last of the five named phases. New `ArcFSAttrTypeString()`=6/`ArcFSAttrTypeBlob()`=7 type codes; `ArcFS.SetStringAttribute`/`GetStringAttribute`/`SetBlobAttribute`/`GetBlobAttribute` (the Blob pair a direct, symmetric reading of Section 12.1's own "same representation, only the type code differs" design, beyond Section 12.2's own literal text which named only the String pair). Zero on-disk Attribute Tree format change: `valueLow`/`valueHigh` change MEANING for these two type codes (a data-pool slot index and a real byte length) rather than width, reusing the existing `ArcFSAllocateDataSlot` allocator a FILE's own content already uses, capped at `ArcFSFileCapacityBytes()` (4096 bytes, one chunk) per value. `ArcFSGatherUniqueDataSlots` was the one function needing a real extension (a second pass over active Attribute rows, feeding the SAME unique-slot list a FILE's own dataSlot already populates) -- confirmed by re-reading every other stage of the Extent Tree build/count/gather pipeline that none of them needed any change, since they are all already generic over "a data-pool slot with touched chunks," regardless of owner. A real design flaw caught before any QEMU run, not discovered as a live bug: `ArcFSPopulateObjectRow`'s own row-index-as-live-slot-identity trick (safe for files, since the object-row-index domain and data-pool-slot domain are the SAME size) does not generalize to attribute rows, whose index domain (`ArcFSMaxAttributes()`=4096) is larger than the data-pool-slot domain (`ArcFSMaxObjects()`=2048) since Phase Q -- fixed by allocating a genuinely fresh live slot per attribute row at mount time instead. That fix exposed a second real bug: `ArcFS.MountImage`'s own data-slot bump-counter update ran too late (after `ArcFSLoadAttributes`, not before), which would have let a fresh mount-time allocation collide with a slot an already-loaded object row still owned -- fixed by moving the bump to run immediately after `ArcFSLoadObjects` succeeds. Real QEMU/OVMF proof (`aps-arcfs-string-blob-attribute.abas`): a genuine multi-byte UTF-8 string (1/2/3-byte sequences, not ASCII-only) and a genuine 37-byte binary blob (full 0..255 byte range, real `0x00` mid-buffer, real `0xFF`) both survive a real commit+remount byte-for-byte, coexisting with a pre-existing, completely undisturbed integer attribute on the same object; also proves the size cap is refused outright (no attribute row left behind) and that `GetStringAttribute` fails closed on an insufficient destination buffer. Passed first attempt; negative control confirmed real `FAIL 1`; deterministic across 3 runs. New smoke test `systems_arco_basic_arcfs_string_blob_attribute_smoke`; full suite re-run -- see this entry's own report for the final count. See `.agents/reports/aps-arcfs-phase-r.md`. All five named phases (N-R) now implemented and QEMU-proven; the Allocation Bitmap's own multi-sector redesign remains real, honestly-named future work beyond this RFC's five named phase letters. |
