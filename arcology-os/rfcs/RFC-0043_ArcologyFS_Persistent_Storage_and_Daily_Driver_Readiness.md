...
| 0.5     | 2026-08-22 | Phase W (Section 9, long-session durability) implemented and QEMU-proven. Three real findings surfaced by actually building the proof, not smoothed over: (1) a real, pre-existing bug in `ArcFS.ReclaimableSectorCount()` -- never checked `ArcFSBitmapGet(sector) <> 0` before counting a sector reclaimable, unlike `ArcFS.ReclaimGeneration()`'s own correct logic, since RFC-0039 Phase E, never exercised this way before; fixed with the same guard. (2) A real design-timing correction: a reclaimable-sector check is only meaningful immediately after `ArcFS.ReclaimGeneration()`, in memory, before the following persist commit -- that commit itself always creates a fresh bitmap+checkpoint span and supersedes whatever generation was active during the reclaim, making THAT generation's own metadata newly reclaimable; no number of additional reclaim+commit rounds ever reaches a persisted, checkable zero. (3) A real, honestly-named architectural finding, not a bug: `ArcFS.Remove` only unlinks a namespace entry, never freeing the underlying Object Table row -- a "removed" object becomes a real orphan (correctly detected by the existing Pass 4 scrub), and this backend has no real object deletion at all today, only `ArcFS.RepairReattachOrphans`'s own relocate-into-`:lost+found` remedy -- a long enough create+remove session will eventually exhaust `ArcFSMaxObjects()` regardless, named as real future work. Also measured directly (not estimated): this backend's own O(high-water-mark) reclaim/allocation cost, worsened by Phase U's own much larger per-commit bitmap span, makes RFC-0043 Section 9's own literal "dozens" of cycles infeasible under QEMU's software (TCG) emulation within any practical test-suite budget -- 4 real cycles is the honest, empirically-verified number this increment ships, down from measured progression 0 cycles ~15s/1~31s/3~80s/6 exceeded 280s without completing. Real QEMU/OVMF proof (`aps-arcfs-long-session-durability.abas`): a real anchor object survives 4 real create/write/commit/remove/reclaim cycles plus two mid-session reclaim checkpoints and a final real repair pass, verified via a real remount with content read back byte-for-byte. Passed after all three fixes above (isolated one at a time via a debug build with per-check serial markers); negative control confirmed real `FAIL 1`; deterministic across 3 runs (~89s/67s/67s guest CPU). New smoke test `systems_arco_basic_arcfs_long_session_durability_smoke` (151.04s, the slowest test in the suite, a disclosed real cost); full suite re-run -- see `.agents/reports/aps-arcfs-phase-w.md` for the final count. |
| 0.6     | 2026-08-22 | Phase X (Section 10, physical-hardware validation package) delivered. `.agents/reports/WP-029-arcfs-persistence-hardware-validation.md`, matching `.agents/reports/WP-026-hardware-validation-package.md`'s own established template exactly -- build identity, real SHA-256 checksums for both `.EFI` files and both wrapped removable-media images, test-platform fields, a two-stage checklist (write the writer image, boot, confirm; a REAL cold power-off, not a warm reboot; write the reader image to the same boot media, boot again with the same data disk attached, confirm), observation and sign-off sections. Reused `arcology-os/scripts/build/build-arcology-hardware-image.py` (this project's own existing, byte-reproducible FAT32 image builder, originally built for WP-024/WP-026's own `arcology-seed-0.1` artifact) directly, unmodified, with RFC-0043 Phase T's own existing writer/reader fixtures as input -- no new build tooling needed. A real self-corruption incident during preparation, found and explained, not hidden: the first QEMU sanity-check attempt (a single-drive boot, no second disk attached) let the writer fixture discover and format its OWN boot media via `UefiBlockDevice.Discover`'s own documented "returns whichever handle comes first" behavior, overwriting the built FAT32 image with a live ArcFS superblock; traced through, the image-building tool itself was independently confirmed correct via a byte-for-byte diff against a fresh rebuild, and the artifacts were rebuilt clean. That incident surfaced a real, genuine safety finding for the package itself, now documented explicitly within it: real hardware's own USB/SATA/NVMe enumeration order cannot be controlled the deterministic way QEMU's own PCI `addr=` ordering controls it (which is how Phase T's own proof avoids this), so a human tester must treat the boot medium as fully disposable. The real sanity check, executed under QEMU/OVMF using the ACTUAL wrapped images (not the loose `.EFI` files) as boot media: a real two-drive run confirmed the writer image genuinely formats a real, separate data disk (`ARCFSB02` read directly off the resulting file) while leaving its own boot media's FAT32 structure untouched, and the reader image, run as a separate QEMU process against that same data disk, reports `PERSIST READER DONE` -- explicitly named in the package as a sanity check on the artifact bytes, not a substitute for the real physical checklist. Status stays `READY FOR HUMAN EXECUTION -- NOT YET VALIDATED`, matching this project's own standing rule that no physical-hardware claim is ever inferred from QEMU. See `.agents/reports/aps-arcfs-phase-x.md`. With Phase X delivered, all six named RFC-0043 phases (S, T, U, V, W, X) are complete. |
