...
- Real block-device drivers: AHCI, NVMe, virtio-blk (QEMU-native, useful for faster iteration than
  AHCI emulation during this project's own development). **virtio-blk is now delivered** (rev 1.3,
  see Revision History): real discovery/read/write against a genuinely attached QEMU disk, plus a
  `BlockDevice`-contract-shaped stdlib provider (`UefiBlockDevice.*`). AHCI/NVMe remain
  unaddressed, though the underlying compiler binding (`EFI_BLOCK_IO_PROTOCOL` itself) does not
  differ by transport, so no new compiler work is expected there — only a QEMU harness variant.
...
| 1.1     | 2026-08-19 | The `STRING`-equality gap flagged in v1.0 is now fixed at the root (not just routed around) -- see `.agents/reports/freestanding-string-equality.md`. `=`/`<>` on `STRING` operands now compares UTF-16 text content via a hand-assembled comparison loop in `src/compiler/fission.cpp`'s `Binary`-operator codegen; a second, independent encoder bug the fix surfaced (`mov_load16_rax`'s incorrect `0x66` prefix, `arco/x86_64_encoder.hpp`) is fixed too. This RFC's own shipped 8.3/mount-point byte buffers were deliberately left as-is (already proven, no reason to churn them) -- the fix removes the need for *future* freestanding work to avoid `STRING`, not a requirement to revisit this RFC. |
| 1.2     | 2026-08-21 | Section 17.5's own named stop condition -- "the boot-media RAM-disk-population step needs UEFI protocol surface this project's frontend does not yet bind" -- is now closed at the compiler-binding level: `EFI_BLOCK_IO_PROTOCOL`/`EFI_BLOCK_IO_MEDIA` are bound (`include/arco/uefi_bindings.hpp`, documented in `uefi-bindings.md` following its own established conventions), with a new `UEFI.BLOCKIO.DISCOVER` intrinsic mirroring `UEFI.GOP.DISCOVER` exactly (GUID and offsets verified against `MdePkg/Include/Protocol/BlockIo.h`, cross-checked byte-for-byte in generated machine code), plain `MediaId`/`MediaFlags`/`BlockSize`/`LastBlock` field accessors, and real `ReadBlocks`/`WriteBlocks`/`Reset`/`FlushBlocks` protocol methods reachable through the already-generic `CallExternal` codegen with **zero new calling-convention code** -- confirmed in generated x86-64 (`call [r11+0x18]` at the exact registered `ReadBlocks` offset). Proven via `systems_blockio_discovery_smoke` (AST/A-MIR/X86_64 structural + machine-code verification, `arcology-os/tests/fixtures/blockio-discovery/`), the same tier `systems_gop_discovery_smoke` already established for GOP. **Not yet done, and explicitly out of scope for this increment**: an ArcoBASIC `BlockDevice`-shaped stdlib provider backed by real discovered hardware (`RAMDisk.*`'s own concrete-function pattern cannot be copied directly -- calling `.ReadBlocks`/etc. requires the typed `UEFI.BlockIoProtocol` value to be an actual function *parameter* at each call site, per `CallExternal`'s parameter-rooted-chain constraint, unlike RAMDisk's fully hidden singleton state; a real provider needs to thread `blockIo` through explicitly rather than hiding it), and a QEMU test-harness variant that attaches a genuine emulated disk (virtio-blk/AHCI via `-drive`) rather than the existing RAM-preload trick, for a true end-to-end hardware-discovery-to-read proof. Both tracked as the concrete next increment. See `.agents/reports/aps-blockio-discovery-binding.md`. |
| 1.3     | 2026-08-21 | Both items v1.2 deferred are now delivered. A new QEMU harness (`run-uefi-hello-with-blockio-disk.sh`) attaches a REAL, genuinely separate virtio-blk disk (built by `build-blockio-test-disk.py`) alongside the boot media -- two real, empirically-found requirements were needed to make discovery land on the intended disk deterministically: `-nodefaults` (without it, QEMU's default auto-added empty IDE CD-ROM is what `LocateProtocol` returns first) and explicit PCI `addr=` ordering (OVMF's PCI bus walk installs BlockIo handles in ascending address order, confirmed empirically). `UefiBlockDevice.*` (`stdlib/uefi_block_device_policy.abas`) now matches RFC-0038 Section 6.1's `BlockDevice` contract shape (`SectorSize`/`SectorCount`/`IsWritable`/`ReadSectors`/`WriteSectors`), genuinely backed by real discovered hardware -- proven under real QEMU: real discovery, real geometry (`SectorSize=512 SectorCount=128`, matching the actual test disk), Requirement 6.1's out-of-bounds rejection, a real read recovering a known magic pattern, and a real write+read-back round trip, all through the provider itself, plus a negative control confirming the checks are real (not vacuous), deterministic across 3 runs. A design-constraint CORRECTION from v1.2: the earlier claim that a discovered `UEFI.BlockIoProtocol` value must be threaded through as an explicit function parameter at every call site was an incomplete reading of the compiler's own resolution code -- a local variable reloaded from memory and re-declared with an explicit `AS UEFI.BlockIoProtocol` type annotation resolves identically (confirmed directly in generated machine code), which is what makes `UefiBlockDevice.*`'s hidden-scratch-state design possible at all. See `.agents/reports/aps-blockio-disk-provider.md`. Not wired into `fat32_policy.abas`/`volumes_policy.abas` -- `RAMDisk.*` remains those modules' block device; `UefiBlockDevice.*` is a parallel, equally-real provider, not yet a drop-in replacement. |
| 1.4     | 2026-08-21 | `UefiBlockDevice.*` is now wired into `fat32_policy.abas` as a genuine, selectable ALTERNATIVE to `RAMDisk.*` -- `FAT32.SelectBlockDevice(provider)` picks between them (default `RAMDisk`, zero breaking change for every existing caller), dispatched through a fixed two-way `IF`-chain (`FAT32BlockReadSectors`/`FAT32BlockSectorCount`), the SAME idiom `loop_policy.abas`'s `LoopInvokeSlot` and `volumes_policy.abas`'s own provider-tag dispatch already established for "pluggable behavior without a real interface value" on this backend -- not a new pattern. Both providers implement RFC-0038 Section 6.1's identical `BlockDevice` contract shape by construction, so the dispatch is a clean wrapper, not a papered-over mismatch; selecting `UefiBlockDevice` without discovering it first fails closed by construction too (`SectorCount()` reads 0 from untouched scratch state, so `ReadSectors`'s own bounds check rejects every request), confirmed directly under real QEMU, not assumed. Real proof: `FAT32.Mount()` against a real attached disk with the default provider genuinely fails closed; after real discovery + selection, the SAME `Mount`/`Open`/`Read`/`Close` sequence against the SAME disk succeeds, recovering a real multi-cluster file (`README.TXT`, 3333 bytes, byte-sum 211490) byte-for-byte -- the identical known-good file/checksum the ORIGINAL RAMDisk-backed proof (`aps-block-storage.abas`) already established, proving the read path itself is unchanged. Negative control (corrupted file content) confirmed real; deterministic across 4 runs. `volumes_policy.abas` untouched and out of scope (provider-tag resolution is a separate layer above which `BlockDevice` `FAT32.*` reads through). See `.agents/reports/aps-blockio-fat32-uefi.md`. |
