...
**Status:** Draft — Phases 1-4 implemented and QEMU-proven (PS/2 driver, xHCI controller driver,
USB HID keyboard driver, substrate unification with a real GOP-framebuffer terminal and boot
dashboard). Phase 5 (real hardware validation, WP-033) is in progress: the real freeze this
project's own hardware hit once (0.7) is confirmed FIXED on real hardware, and the boot dashboard
(0.8) is confirmed rendering there too. A real terminal-font legibility finding from that same test
was fixed in 0.9, not yet re-validated on real hardware. Status stays Draft until a human completes
the full WP-033 checklist.
...
| 0.7 | 2026-08-23 | Phase 5 package (WP-033) built and written to the user's own real USB stick for the first time. Real hardware froze at `SUBSTRATE TABLE OK`; diagnosed via raw pixel-block checkpoints (bypassing the font/terminal system entirely) at 6 risk points, confirmed via direct user observation (red/orange/yellow visible, then a 2-minute freeze, never reaching green) that the failure was `CPU.WriteCR3(root)` itself — the first time `ExitBootServices` + CR3/GDT/IDT takeover ran on real silicon anywhere in this project; all prior proof was QEMU-only. Root cause: the identity map only covered the low 1GB plus two dynamically-discovered regions; QEMU happens to load images within that 1GB (an untested assumption this project never noticed making), but this laptop loads the image/stack elsewhere, faulting the instant CR3 switched away from firmware's own page tables. Fixed by identity-mapping the full 512GB PML4-slot-0 range using the same already-allocated PDPT0 page (entry 0 keeps the existing low-1GB map, entries 1-511 become direct 1GB pages) — zero extra allocation cost. Also brightened the orange checkpoint color per direct user feedback ("kinda looks red"). Fix not yet re-validated on real hardware as of this entry. |
| 0.8 | 2026-08-23 | Implemented the user's own "Arcology Boot Screen Concept" design spec (`docs/arcology-boot-screen-concept.md`) as a real System Assembly View dashboard replacing the substrate's plain-text boot markers: header, a UEFI→APS authority-transition diagram (the APS box turns magenta and its link to UEFI visibly severs at the real `ExitBootServices`→CR3 moment), six subsystem panels with real per-panel state (MEMORY/ADDRESS SPACE/I-O/INPUT/DISPLAY/CONSOLE, substituted for the mockup's own ArcFS since this fixture doesn't touch ArcFS), a segmented assembly bar, an 8-slot event feed, and a status panel — all driven by this fixture's own real boot milestones, not simulated progress. Two real findings: (1) a function with **no locals**, just ~670 sequential `MEMORY.Write8` calls, still exceeded the backend's stack-frame budget from accumulated per-statement expression temporaries — confirming this limit isn't purely locals-driven; fixed by splitting into chunk functions (eventually 40 statements per chunk, smaller than an earlier 150-per-chunk attempt that still failed). (2) firmware's own `ConsoleOut` text cursor, still advancing independently after the dashboard's own drawing had started, landed glyphs from two pre-`ExitBootServices` diagnostic markers (`SUBSTRATE USB READY`/`NONE`, `SUBSTRATE TABLE OK`) directly on top of already-painted dashboard panels — confirmed via a real QEMU monitor `stop`+`screendump` mid-boot capture; fixed by suppressing those two markers once the dashboard is live (the event feed already carries the same milestones, labeled). Verified via the existing `systems_aps_arcology_seed_substrate_smoke` (unchanged, still passing) plus a new manual mid-boot screendump check; 96/96 full regression suite. WP-033 rebuilt against this artifact (new checksums) — the pre-dashboard image previously written to the user's real USB stick does not cover this build; a re-write is still needed before Phase 5's own checklist can be completed. |
| 0.9 | 2026-08-23 | First real physical test of the 512GB identity-map CR3 fix (0.7) and the boot dashboard (0.8), written to the user's real USB stick and confirmed working — the freeze is gone, `HELP`/`OT` both work, and the dashboard "looks nice and unique." One real new finding: the terminal's `READY.` text (an unrelated, separate rendering path from the dashboard) was "nearly impossible to read" — a fixed 8x8-at-2x (16px) glyph cell, legible on QEMU's own 1280x800 virtual display, doesn't track a real panel's own actual resolution. Fixed by computing `TermGlyphScale` once at boot from the real detected GOP width via explicit breakpoints (1280→2, 1600+→3, 2200+→4, 3000+→5, 3800+→6) instead of a hardcoded constant; `TermCellPx` now derives from it. An earlier version of this fix used a target-column-count-via-integer-division formula that silently floored a real common width (1920) right back down to the same illegible scale=2 it was meant to fix (1920 \ 90 \ 8 = 1280 \ 90 \ 8 = 2) — caught before commit by testing at a real QEMU-simulated 1920x1080 (`-device VGA,edid=on,xres=1920,yres=1080`) and actually looking at the screendump, not just trusting the formula. QEMU's own default 1280-width behavior deliberately stays byte-for-byte identical (confirmed via an unchanged bright-pixel-fraction reading in the existing pixel-region smoke test) so no existing proof shifted. 95/96 full regression suite — the one failure is the same pre-existing, unrelated `systems_ps2_keyboard_driver_smoke` flake noted in 0.3, confirmed via isolated re-run to be untouched by this change. |
