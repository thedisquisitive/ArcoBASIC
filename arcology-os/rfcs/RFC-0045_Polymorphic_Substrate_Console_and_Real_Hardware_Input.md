...
dashboard). Phase 5 (real hardware validation, WP-033) is the only phase left: a real freeze this
project's own hardware has already hit once was diagnosed and fixed (see Revision History 0.7),
but that fix has not yet been re-validated on real hardware, and the dashboard added in 0.8 has
never been tested on real hardware at all. Status stays Draft until a human completes WP-033.
