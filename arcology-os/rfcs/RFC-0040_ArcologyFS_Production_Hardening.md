...
| 1.3     | 2026-08-21 | Closes two more named gaps: the Attribute Tree and Extent Tree both now have real defect-tolerant scrub coverage, wired into `ArcFSScanGeneration` as Pass 6/7. `ArcFSAttributeTreeCollectAllEntriesTolerant` already existed since Phase L but was never actually called from anywhere outside its own recursion -- dead code from the caller's perspective, now genuinely wired in. A new `ArcFSExtentTreeCollectAllEntriesTolerant` was written (the Extent Tree had no tolerant variant at all), mirroring the Object Tree's own tolerant walk exactly. `ArcFSPeekCheckpoint` gained `extentTreeCount` (peek offset 88) alongside the `extentTreeRoot` the previous increment already added, needed to cross-check Pass 7's own collected count the same way every other pass already does. A REAL FINDING while writing the fixture, not a bug: a corrupted tree node correctly produces `Corrupt` (5), not `Degraded` (1) -- `ArcFS.GetHealthState()`'s own classification already treats ANY nonzero structural-defect count as `Corrupt` before `Degraded` is ever considered, matching the identical convention Pass 1/2 already established for the Object/Namespace Trees (Phase M's own report: "a real corrupted tree-node sector detected as Corrupt"). The fixture's own first-draft expectation was wrong, not the implementation -- corrected in the fixture, nothing changed in `arcfs_policy.abas` as a result. Real QEMU/OVMF proof (`aps-arcfs-tree-scrub-coverage.abas`): a file with real multi-chunk content AND a real attribute scrubs Healthy after a real commit+remount (neither new pass false-positives on legitimate data in both trees at once); directly corrupting the Attribute Tree's own root node on the BlockDevice makes `ArcFS.GetHealthState()` genuinely report Corrupt, restoring the bytes returns Healthy; the identical corrupt/restore cycle against the Extent Tree's own root node proves Pass 7 the same way, with file content confirmed byte-for-byte correct afterward. Negative control confirmed real `FAIL 1`; deterministic across 3 repeated runs. New smoke test `systems_arco_basic_arcfs_tree_scrub_coverage_smoke`; full suite re-run clean. Every tree this format defines (Object, Namespace, Attribute, Extent) now has both real reachability coverage and real defect-tolerant scrub coverage. See `.agents/reports/aps-arcfs-tree-scrub-coverage.md`. |
| 1.4     | 2026-08-21 | Closes the last major named structural gap in Phase L: the Attribute Table is now genuinely `(OID, attributeId)`-keyed (Section 12.1's own "multi-attribute-per-object model"), not one-slot-per-object -- a second `ArcFS.SetAttribute` on the same object no longer silently clobbers the first. `attributeId` is a caller-defined integer, not the RFC's literal namespace-plus-name string pair, this file's own already-established "use integer IDs instead of strings" pattern (freestanding STRING still has no LEN/MID). Row layout widened 24->56 bytes (`oidLow`/`oidHigh`/`attributeId`/`active`/`type`/`valueLow`/`valueHigh`); `ArcFSMaxAttributes()` (128) is independent of `ArcFSMaxObjects()` for the first time. New `ArcFSFindAttributeRow`/`ArcFSFindFreeAttributeRow` helpers; every public function (`SetAttribute`/`HasAttribute`/`GetAttributeType`/`GetAttribute`/`GetAttributeHigh`) gained an `attributeId` parameter; new `ArcFS.RemoveAttribute(oid, attributeId)`, meaningful for the first time now that clearing one attribute no longer has to clobber an object's others. On-disk Attribute Tree entry width widened 40->48 bytes, touching the gather/build/collect functions and `ArcFSLoadAttributes`'s own offset math; `ArcFSPopulateAttributeRow` redesigned to write at the tree walk's own sequential `index` (matching `ArcFSPopulateObjectRow`'s own convention) instead of the owning object's row, since one object can now own more than one row. Attribute Tree scratch addresses relocated (two pages, not one, for the wider entries) into the large free gap before `ArcFSDataPoolAddress`. Real QEMU/OVMF proof (`aps-arcfs-multi-attribute.abas`): object A gets TWO distinct attributes at once (impossible under the old model); object B gets its own `attributeId=1`, proving rows key on the FULL pair, not `attributeId` alone; `ArcFS.RemoveAttribute` clears exactly one row, leaving A's other attribute and B's attribute both intact; a commit+remount confirms the removal persists and the survivors round-trip byte-for-byte through a real on-disk tree walk with `ArcFS.GetHealthState()` reporting Healthy; a SECOND commit+remount re-adds a fresh attribute at the vacated `(oid, attributeId)`, proving the freed row is genuinely reusable across committed generations, not just within one session. Negative control confirmed real `FAIL 2`; deterministic across 3 repeated runs. New smoke test `systems_arco_basic_arcfs_multi_attribute_smoke`; full suite re-run clean, including the 3 pre-existing frozen fixtures that reveal-check the OLD attribute-function signatures (unaffected -- `reveal --entry` compiles a function's own definition, not a call site, and every frozen fixture carries its own independent stdlib copy anyway). See `.agents/reports/aps-arcfs-multi-attribute.md`. |
