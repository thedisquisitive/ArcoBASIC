...
**Status:** Draft (Phases A-F implemented and QEMU-proven; Phase G partially addressed -- update-snapshot rollback implemented and QEMU-proven, ArcoBASIC bindings recognized as already satisfied by construction, and namespace attachment/system volume use/recovery environment support/graphical storage tooling explicitly left open pending other Arcology subsystems this repository does not yet implement -- see `.agents/reports/aps-arcfs-phase-a.md` through `aps-arcfs-phase-g.md`. Deliberately not marked Implemented: see Phase G's own report for the full list of accumulated, honestly-documented scope reductions across all seven phases)
