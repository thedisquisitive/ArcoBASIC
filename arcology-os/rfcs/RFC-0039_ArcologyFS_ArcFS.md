...
**Status:** Draft (Phases A-F implemented and QEMU-proven; Phase G addressed for five of its six items -- update-snapshot rollback, system-volume activation with a self-healing boot policy, the Arcology System Namespace with real ArcFS attachment (RFC-0041), a distinct recovery-environment boot artifact staging Inspect/Apply/Verify, and ArcoBASIC bindings recognized as already satisfied by construction, all implemented and QEMU-proven; only graphical storage tooling remains explicitly open, alongside real UEFI hardware storage discovery which stays RFC-0038's own future work -- see `.agents/reports/aps-arcfs-phase-a.md` through `aps-arcfs-phase-g.md`. Deliberately not marked Implemented: see Phase G's own report for the full list of accumulated, honestly-documented scope reductions across all seven phases)
