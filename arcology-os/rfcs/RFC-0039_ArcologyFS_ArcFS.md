...
**Status:** Draft (Phases A-G all implemented and QEMU-proven -- Phase G complete: update-snapshot rollback, system-volume activation with a self-healing boot policy, the Arcology System Namespace with real ArcFS attachment (RFC-0041), a distinct recovery-environment boot artifact staging Inspect/Apply/Verify, a real GOP-framebuffer graphical storage inspector, and ArcoBASIC bindings recognized as already satisfied by construction; the one residual exception is real UEFI hardware storage discovery, which stays RFC-0038's own separately-tracked future work, not an ArcFS gap -- see `.agents/reports/aps-arcfs-phase-a.md` through `aps-arcfs-phase-g.md`. Deliberately not marked Implemented: see Phase G's own report for the full list of accumulated, honestly-documented scope reductions across all seven phases)
...
| 0.4     | 2026-08-19 | Section 78 Phase C ("formatter and transactional writer... allocation tree; copy-on-write metadata; commit protocol; durable checkpoint publication; crash-injection tests") implemented and validated end-to-end under QEMU/OVMF: `ArcFS.FormatVolume()` writes a brand-new generation-1 volume to a real `BlockDevice`; `ArcFS.PrepareCommit()`/`ArcFS.PublishCommit()`/`ArcFS.CommitImage()` implement Section 15.2's commit protocol, writing every object/namespace/data record a commit needs to fresh sectors and publishing the new generation with one single-sector superblock write -- proven copy-on-write by re-reading a prior generation's own checkpoint sector byte-for-byte unchanged after a later commit, and proven atomic (Section 75.5) by a crash-injection fixture that calls `PrepareCommit` without ever calling `PublishCommit` and shows the old generation remains the only one any subsequent mount can see. Five documented Phase C scope reductions (bump-allocator, not a real allocation tree; flush as a no-op; no intra-transaction rollback; no multi-writer concurrency; placeholder volume UUID). Status remains Draft -- Phases D-G still ahead. See `.agents/reports/aps-arcfs-phase-c.md`. |
| 0.5     | 2026-08-19 | Section 78 Phase D ("mutation completeness... create/write/resize; rename/move; delete; sparse files; reflinks; typed attributes") implemented and validated end-to-end under QEMU/OVMF: `ArcFS.Resize()` grows (zero-filling, Section 19) and shrinks a file within its existing fixed capacity; `ArcFS.Reflink()` (Section 22) creates a new OID that shares the source's data-pool slot with nothing copied at reflink time, made safe by decoupling data-slot allocation from object-row allocation and adding a per-slot reference-count table -- the first write through either OID afterward triggers copy-on-write via a new shared gate (`ArcFSEnsurePrivateSlot`) that both `HandleWrite` and `Resize` now pass through, proven by reading the un-written side immediately after a write to the other and finding it byte-for-byte unaffected. Rename/move/delete (Phase A's own APIs, unchanged) and the new typed-attribute API are exercised and carried through a real commit + remount cycle, positively proving durability for the former and the documented non-persistence for the latter. Four documented Phase D scope reductions (no sparse files; no on-disk reflink sharing, only the in-memory copy-on-write semantic; attribute values are in-memory only, not yet part of the checkpoint format; no advance capacity query). One general fix: `ArcFS.PrepareCommit` (Phase C) had assumed dataSlot always equals row index; generalized to read the row's own field, byte-identical output for every pre-Phase-D object, confirmed by re-running Phases A-C unchanged. Status remains Draft -- Phases E-G still ahead. See `.agents/reports/aps-arcfs-phase-d.md`. |
| 0.6     | 2026-08-19 | Section 78 Phase E ("snapshots and reclamation... snapshot roots; generation pinning; shared extent accounting; safe reclamation") implemented and validated end-to-end under QEMU/OVMF: `ArcFS.CreateSnapshot()`/`ArcFS.DeleteSnapshot()` (Section 21) pin and unpin the active checkpoint so it survives later commits; `ArcFS.IsSectorReachable()`/`ArcFS.ReclaimableSectorCount()`/`ArcFS.SnapshotRetainedSectorCount()` make "generation pinning" and "shared extent accounting" concrete -- since Phase C's bump allocator never reuses a sector, nothing has ever actually been destroyed, so this phase's real deliverable is the safety analysis a future reclaiming allocator would need before it is EVER allowed to reuse a sector, proven correct now. `ArcFS.SnapshotResolve()`/`ArcFS.SnapshotReadFile()` read a pinned generation's own on-disk content directly, deliberately never touching the live write-path tables (an early draft that reused `ArcFS.MountImage()`'s tables for this was rejected as corruption-prone) -- proven by reading a snapshot's content back correctly after the live tree diverged from it twice. Four documented Phase E scope reductions (no real space reuse -- the allocator itself is untouched; fixed 8-slot snapshot table; no snapshot metadata beyond generation+checkpoint sector; O(sectors) reclaim scan, not an interval index). One bug caught before shipping: an early `ArcFS.SnapshotReadFile` draft read a full 4096-byte extent straight into a caller buffer smaller than that, an out-of-bounds write; fixed with a dedicated extent scratch buffer. Status remains Draft -- Phases F-G still ahead. See `.agents/reports/aps-arcfs-phase-e.md`. |
| 0.7     | 2026-08-19 | Section 78 Phase F ("recovery and health... scrub; offline checker; repair planning; ReadOnlySafety activation; inspector health interface") implemented and validated end-to-end under QEMU/OVMF: `ArcFSScanGeneration` re-verifies every object/namespace checksum, performs a real from-root graph reachability analysis (not merely "does some record name me as a child"), and detects overlapping file extents; `ArcFS.ScrubVolume()`/`ArcFS.GetHealthState()` (Sections 20.4/36) build on it. `ArcFS.MountImageSafe()` (Sections 37/38) activates ReadOnlySafety with real teeth -- `ArcFS.PrepareCommit()` now checks the flag first and genuinely REFUSES ordinary mutation while it is armed, proven by creating a file in memory during ReadOnlySafety and confirming the commit itself is rejected, not just that a status value says "degraded." `ArcFS.RepairReattachOrphans()`/`ArcFS.RepairCommit()` (Section 39) implement Inspect/Plan/Apply/Verify for exactly one defect class (orphaned objects, reattached into a root-level `:lost+found` with a synthetic `orphan-<oid>` name -- the OID itself is the provenance record), chosen because it is the one repair policy RFC-0039 explicitly names; RepairCommit re-verifies a clean scan after its own commit and re-arms ReadOnlySafety if a defect remains, rather than trusting the repair blindly. Five documented Phase F scope reductions (health state collapsed to three values; only one defect class has a repair; no scrub/repair history persisted; O(n^2) scan; extent-overlap detection code-reviewed but not exercised by a dedicated corrupt-image fixture). Status remains Draft -- Phase G still ahead. See `.agents/reports/aps-arcfs-phase-f.md`. |
| 0.8     | 2026-08-19 | Section 78 Phase G ("system integration... namespace attachment; system volume use; recovery environment support; update snapshots; graphical storage tooling; ArcoBASIC bindings") partially addressed, honestly: `ArcFS.RollbackToSnapshot()` (Section 51's "update snapshots") implemented and validated end-to-end under QEMU/OVMF -- reuses `ArcFS.PublishCommit()`'s exact single-sector-superblock-write mechanism to republish a previously snapshotted generation as active, a real atomic undo (not a restore-by-copy), proven by rolling back a bad update and confirming the ORIGINAL content returns exactly, then confirming the volume is fully read-write usable afterward (a new file commits and round-trips on top of the rolled-back tree). ArcoBASIC bindings recognized as already satisfied by construction (every `ArcFS.*` function already is a native ArcoBASIC entry point; this compiler has no separate FFI layer for a "binding" to occupy). The remaining four Phase G items (namespace attachment, system volume use, recovery environment support, graphical storage tooling) are explicitly left OPEN, each with its missing dependency named -- none of the Arcology subsystems they would integrate with exist anywhere in this repository yet (matching Section 80's own dependency list and Phase A's own RFC-0017 finding, independently re-confirmed). Building fake infrastructure to claim them "done" would violate Section 79.2's own "No silent architectural substitution" rule. RFC-0039's Status stays Draft, deliberately -- see `.agents/reports/aps-arcfs-phase-g.md` for the full accumulated-scope-reduction summary across all seven phases. |
| 0.9     | 2026-08-19 | RFC-0040 (ArcologyFS Production Hardening) created, covering every ArcFS-owned gap named across Phases A-G's own reports: real free-space allocation/reclamation, growable B+tree metadata trees, 128-bit OIDs, variable-length/sparse files, on-disk reflink sharing, persistent attributes, a full seven-state health model, and expanded repair coverage, under a new Format Major Version 2. Explicitly excludes RFC-0039 Phase G's remaining namespace-attachment/system-volume/recovery-environment/graphical-tooling items, matching that phase's own finding that their dependencies do not exist in this repository. No implementation has begun; see RFC-0040 itself. |
| 0.10    | 2026-08-19 | Phase G addendum: "system volume use" narrowed and delivered. `ArcFS.ActivateSystemVolume()` implements Section 37's activation checklist and Section 38's ReadOnlySafety as a real, self-healing boot policy -- validated end-to-end under QEMU/OVMF across all three real outcomes in one continuous boot session: a healthy volume activates read-write directly; a volume with the same orphan defect Phase F's own fixture builds self-heals during activation and still comes up read-write, with the reattached object confirmed to be the SAME object (same OID, same content); a volume with its superblock destroyed reports Unavailable rather than guessing. Real UEFI Block IO discovery of physical storage remains explicitly out of scope, per RFC-0038 Section 17.5's own stop condition against improvising new protocol bindings as a side effect of unrelated work -- that half of "system volume use" is unchanged, still open. Status line updated accordingly; Phase G's own report carries the full addendum. See `.agents/reports/aps-arcfs-phase-g.md`. |
| 0.11    | 2026-08-20 | Phase G addenda 2 and 3: "namespace attachment" and "recovery environment support" delivered. RFC-0041 (Arcology System Namespace) closes namespace attachment -- a real System Namespace attaches an ArcFS volume and delegates cross-boundary path resolution to `ArcFS.Resolve` completely unmodified, proven end-to-end under QEMU/OVMF including real file content read back through the delegation. A distinct recovery-environment boot artifact closes recovery environment support -- stages RFC-0039 Section 39's own Inspect/Apply/Verify as three independently serial-reported steps, built entirely from Phase F's existing primitives with zero new ArcFS functions, proven to correctly skip repair on a healthy volume and correctly fix-and-reverify a real defect. Both passed on the first real attempt, deterministic across repeated runs, with negative controls confirmed. Status line updated: only graphical storage tooling and real hardware DISCOVERY (RFC-0038's own future work) remain open in Phase G. See `.agents/reports/aps-arcfs-phase-g.md`, `.agents/reports/arcology-system-namespace.md`, and RFC-0041 itself. |
| 0.12    | 2026-08-20 | Phase G addendum 4: "graphical storage tooling" delivered -- Phase G complete. A real GOP-framebuffer volume health indicator renders two real ArcFS volume states (healthy/degraded) with both a distinct color and a distinct positional mark (Section 59: not color alone), verified by reading its own rendered pixels back. Found and fixed a real environmental gap along the way: this project's only existing QEMU harness disables the display entirely, so GOP had never actually been proven reachable under real execution before (only via generated-code pattern matching); added `run-uefi-hello-with-gpu.sh` (a real `-vga std` device, still headless, reporting over serial since a real display flips OVMF's own ConOut default away from serial). Passed on the first attempt after the harness fix, deterministic across repeated runs, negative control confirmed. All six Phase G items are now addressed; the one residual exception, real UEFI hardware storage discovery, stays RFC-0038's own separately-tracked future work. Status line updated accordingly. See `.agents/reports/aps-arcfs-phase-g.md`. |
