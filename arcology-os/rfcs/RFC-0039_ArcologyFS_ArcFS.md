...
**Status:** Draft (Phases A-F implemented and QEMU-proven; Phase G partially addressed -- update-snapshot rollback and system-volume activation (with self-healing boot policy) both implemented and QEMU-proven, ArcoBASIC bindings recognized as already satisfied by construction, and namespace attachment/recovery environment support/graphical storage tooling explicitly left open pending other Arcology subsystems this repository does not yet implement (real hardware storage discovery similarly remains RFC-0038's own future work) -- see `.agents/reports/aps-arcfs-phase-a.md` through `aps-arcfs-phase-g.md`. Deliberately not marked Implemented: see Phase G's own report for the full list of accumulated, honestly-documented scope reductions across all seven phases)
