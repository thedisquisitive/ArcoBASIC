...
|0.3|2026-08-22|First real-world instantiation of this RFC's own bring-up track against a specific, named reference machine (the user's own Lenovo ThinkPad E15 Gen 2), and a real extension beyond the original text-only banner: a genuine GOP (Graphics Output Protocol, RFC-0014) framebuffer render, filling the full real screen at the real discovered resolution (not a hardcoded size) with a two-tone test card, self-verified by reading real pixels back before the completion marker prints. New fixture `tests/fixtures/render-and-halt/render-and-halt.abas`; new smoke test `systems_render_and_halt_smoke`. Real USB Mass Storage Class boot proven SEPARATELY from generic block-device boot (a genuinely distinct real UEFI driver path) via a new harness, `scripts/run/run-uefi-image-with-usb-gpu.sh` (real `qemu-xhci`+`usb-storage`, not a plain `-drive`) -- booting the ACTUAL wrapped, dd-writable image, not just the loose `.efi`. New validation package `.agents/reports/WP-030-render-and-halt-hardware-validation.md`, matching WP-026's own template, including concrete, correct USB-writing instructions (raw block write only, never a filesystem-extract tool). Status stays READY FOR HUMAN EXECUTION -- NOT YET VALIDATED; this RFC's own original WP-026 package remains unexecuted as of this revision too, so nothing in this project has yet been confirmed on real hardware. See `.agents/reports/aps-render-and-halt.md`.|
|0.4|2026-08-22|**First artifact in the entire Arcology OS project confirmed to run on real physical hardware.** WP-030's image, written to a real USB stick (raw `dd`, byte-for-byte verified against the source checksum after writing) and booted on the user's own Lenovo ThinkPad E15 Gen 2: photographic evidence (`.agents/reports/evidence/WP-030-real-hw-test.jpg`) shows the expected two-tone test card (dark navy-blue field, bright accent rectangle) with `ARCOLOGY RENDER AND HALT DONE` clearly legible -- a marker that can only print after real GOP discovery, mode read, full-screen fill, AND both pixel-readback self-verification checks all succeeded on real hardware. One real, benign finding: the background rendered navy-blue rather than neutral gray, consistent with this panel/GOP driver's real channel order differing from a naive RGB assumption -- exactly the deviation the fixture's own channel-order-agnostic design already anticipated (see its header comment), not a defect. This RFC's own Definition of Done is still NOT fully closed: WP-030's own sign-off remains incomplete pending platform/firmware details, stability-duration, and power-cycle-recovery fields still requiring the human tester's own account (not inferable from one photograph), and WP-026 (this RFC's original, plainer text-banner artifact) remains unexecuted. Status stays Draft. See the updated `.agents/reports/WP-030-render-and-halt-hardware-validation.md` for the full, honest accounting of what is and is not yet confirmed.|
