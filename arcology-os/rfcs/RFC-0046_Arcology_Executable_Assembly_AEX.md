...
**Status:** Draft -- Section 61 Phases 1-14 of 15 implemented and QEMU-proven (binary constants,
bootstrap header validation, chunk directory validation, manifest and component graph parsing,
native x86-64 implementation record, ArcFS-backed code/data mapping, native ABI lifecycle
invocation, interface metadata, capability request evaluation, ArcoBASIC Binding Table
registration, first-stage integrity verification, diagnostic metadata registration, deterministic
negative mutation matrix, combined ArcFS-to-native-activation QEMU load path, plus post-checklist
interface resolution, explicit capability grant-state slices, activation-path enforcement, and
resolved-interface ABT lookup, authority-gated ABT registration, and binding dispatch metadata
lookup, binding dispatch invocation, and deactivate lifecycle entry support); Phase 15 not started
...
|0.3|2026-08-24|Section 61 Phase 4 (manifest and component graph parsing) real and QEMU-proven. Concrete 76-byte fixed MANIFEST content layout (AssemblyID, version, BuildID, required format/ABI versions, RootComponentID, DeclaredComponentIDs, display/canonical names) and 44-byte fixed COMPONENT content layout (ComponentID, OwningAssemblyID, role, lifecycle policy, implementation/interface ID lists, name) fixed. `AEX.ValidateManifest` (`stdlib/aex_format_policy.abas`) resolves the header's own RootManifestID through the chunk directory, bounds-checks the manifest's variable regions with the same overflow-safe accumulation discipline Phase 3 established, confirms RootComponentID genuinely appears in DeclaredComponentIDs (a real graph-consistency check, not just a bounds check), and validates every declared component in turn -- including that each component's own recorded OwningAssemblyID matches the manifest's AssemblyID. Proven against 8 new real byte-exact fixtures under QEMU (`tests/fixtures/aex-manifest-probe/`), covering a valid manifest, an unresolvable RootManifestID, a wrong-typed root chunk, manifest content exceeding its own declared logical length, an undeclared RootComponentID, a declared-but-missing required component, an assembly-identity mismatch between manifest and component, and a component exceeding its own declared logical length. All 8/8 passed on first run; no validator or fixture bugs found this round. Phases 5-15 (native implementation record, capability evaluation, ArcFS-backed loading, native ABI lifecycle invocation, integrity, real hardware validation) remain real, separate, later increments.|
|0.4|2026-08-24|Section 61 Phase 5 (native x86-64 implementation record, Section 15/15.1/16) real and QEMU-proven. Concrete 40-byte fixed IMPLEMENTATION content layout (OwningComponentID, ImplementationType, Architecture, ABI major/minor, RequiredCpuFeatures, Priority, CodeChunkID, DataChunkID) fixed, deliberately deferring Section 15's own loosely-specified "compatibility constraints" field until a real consumer needs it. `AEX.SelectImplementation` (`stdlib/aex_format_policy.abas`) is a real implementation of Section 15.1's Loader Selection: walks a component's own ImplementationIDs list (Phase 4's own component field, previously unread), validates each declared implementation in turn (owning component matches, real type, real bounds -- a malformed record fails the whole selection closed), and among implementations actually compatible with this loader (Type=NATIVE, Architecture=X86_64, exact ABI major match, ABI minor no newer than supported) selects the one with the highest declared Priority, or returns Section 15.1's own required specific implementation-resolution error when none qualify. Proven against 8 new real byte-exact fixtures under QEMU (`tests/fixtures/aex-implementation-probe/`): a single compatible implementation, priority-based selection between two compatible implementations, an architecture mismatch, an ABI-minor-too-new mismatch, an unresolvable implementation chunk ID, a wrong-typed implementation chunk, an implementation exceeding its own declared logical length, and an owning-component mismatch. Two real bugs found and fixed in the fixture-generation tooling itself (not the validator): the fixture generator's own chunk alignment values (8) didn't match its own unpadded chunk layout, producing fixtures that were accidentally misaligned by construction; and the ArcoBASIC test-fixture generator's own short-label padding used a NUL byte instead of a printable space, so the smoke test's own expected-output strings never actually matched the real serial bytes -- both are the same class of finding as Phase 1-3's bad-alignment fixture bug, worth remembering as a recurring pattern in this project's offline-Python-fixture-generation discipline. Phases 6-15 (ArcFS-backed code/data mapping, native ABI lifecycle invocation, interface metadata, capability evaluation, ArcoBASIC binding registration, integrity, diagnostics, fuzzing, QEMU/hardware validation) remain real, separate, later increments.|
|0.5|2026-08-24|Section 61 Phase 6 (ArcFS-backed code/data mapping, Section 34 and Loader Stages 1/13) real and QEMU-proven. `AEX.LoadFromArcFS` opens a real ArcFS object by OID, reads up to the loader's explicit 64 KiB per-assembly Phase 6 policy limit, closes the handle, and returns the real byte count to be used as the loader's independently-known file length. `AEX.MapCodeChunk` and `AEX.MapDataChunk` resolve CODE/DATA chunk IDs through the same validated directory, reject missing or wrong-typed chunks with specific errors, enforce caller-supplied destination capacity before copying, and copy the selected chunk bytes byte-for-byte; `AEX.MapDataChunk(..., dataChunkID=0, ...)` implements this phase's ZERO_INITIALIZED no-data-segment case by zero-filling the destination. Proven by new smoke test `systems_aex_code_data_mapping_smoke` using a composed temporary source from the active `stdlib/arcfs_policy.abas` in-memory object/handle slice, active `stdlib/aex_format_policy.abas`, and `tests/fixtures/aex-code-data-mapping/aex-code-data-mapping-body.abas`: the fixture creates a byte-exact minimal AEX, writes it through ArcFS file handles, reloads it through `AEX.LoadFromArcFS`, validates header/directory/manifest/implementation selection from the loaded bytes, maps CODE and DATA chunks successfully, proves zero-fill behavior, and proves invalid/too-large CODE/DATA requests return the new deterministic Phase 6 errors. Phases 7-15 (native ABI lifecycle invocation, interface metadata, capability evaluation, ArcoBASIC binding registration, integrity, diagnostics, fuzzing, broader QEMU validation, physical hardware validation) remain real, separate, later increments.|
|0.6|2026-08-24|Section 61 Phase 7 (native ABI lifecycle invocation, Section 24 and Loader Stage 14) real and QEMU-proven. Concrete 16-byte fixed LIFECYCLE content layout fixed for the first native ABI slice: OwningComponentID, ImplementationID, and ActivateCodeOffset within the selected implementation's CODE chunk. `AEX.ResolveLifecycleEntry` resolves the selected native implementation, validates its CODE chunk, finds a matching LIFECYCLE chunk, confirms component ownership, rejects missing/out-of-bounds lifecycle metadata with deterministic new errors, and stages the resolved entry offset for activation. The freestanding x86-64 compiler/backend now recognizes `AEX.InvokeNative0(entryAddress AS MMIOPTR) AS U64` as a deliberately scoped AEX lifecycle intrinsic lowered to a register-indirect `call` under the existing Microsoft x64 stack-frame discipline; `AEX.InvokeResolvedLifecycle0` uses that intrinsic to invoke the resolved activation entry. Proven by new smoke test `systems_aex_native_lifecycle_smoke`: a byte-exact AEX fixture validates metadata resolution, maps CODE bytes containing `mov eax, 42; ret`, invokes the mapped native entry under QEMU, verifies the returned `U64` value, and covers invalid lifecycle offset, missing lifecycle chunk, and owner-mismatch rejection. Deliberately not claimed yet: argument/context passing, allocator-backed executable mappings, W^X page-table enforcement, unload/deactivation hooks, interface/capability interpretation, ABT registration, integrity, diagnostics, fuzzing, broader QEMU validation, or physical hardware validation. Phases 8-15 remain real, separate, later increments.|
|0.7|2026-08-24|Section 61 Phase 8 (interface metadata, Section 18 and Section 48) real and QEMU-proven. Concrete 44-byte fixed INTERFACE content layout fixed for the first metadata slice: InterfaceID, OwningComponentID, Direction (PROVIDED or REQUIRED), interface major/minor version, 128-bit ContractID, Flags, and zero Reserved field. `AEX.ValidateInterfaces` (`stdlib/aex_format_policy.abas`) walks every component declared by the root manifest, reads each component's ProvidedInterfaceIDs and RequiredInterfaceIDs lists, resolves every referenced INTERFACE chunk through the same directory, confirms chunk type, fixed-length bounds, chunk/content ID agreement, owner match, expected direction, supported interface version, and reserved-field cleanliness, returning deterministic Phase 8 errors for invalid, out-of-bounds, owner-mismatched, or unsupported-version metadata. Proven by new smoke test `systems_aex_interface_metadata_smoke`: a byte-exact fixture validates one provided and one required interface and covers missing interface chunk, wrong chunk type, owner mismatch, too-short interface record, direction mismatch, and too-new interface version. Deliberately not claimed yet: resolving required interfaces to providers, capability evaluation/granting, interface version-range negotiation, ABT registration, symbol dispatch, integrity, diagnostics, fuzzing, broader QEMU validation, or physical hardware validation. Phases 9-15 remain real, separate, later increments.|
|0.8|2026-08-24|Section 61 Phase 9 (capability request evaluation, Section 19) real and QEMU-proven. Concrete 36-byte fixed CAPABILITY_REQUEST content layout fixed for the first policy slice: CapabilityRequestID, OwningComponentID, Requirement (REQUIRED or OPTIONAL), 128-bit CapabilityID, Flags, and zero Reserved field. `AEX.EvaluateCapabilityRequests` (`stdlib/aex_format_policy.abas`) walks CAPABILITY_REQUEST chunks from the validated directory, confirms fixed-length bounds, chunk/content ID agreement, manifest-declared owning component, valid requirement class, and reserved-field cleanliness. A declaration is still explicitly not a grant: the current loader policy accepts valid optional capability declarations and deterministically denies required capability declarations with `AexErrorRequiredCapabilityDenied` until a real authority/grant database exists. Proven by new smoke test `systems_aex_capability_requests_smoke`: a byte-exact fixture covers optional acceptance, required denial, too-short request, undeclared owner, invalid requirement class, request-ID mismatch, and nonzero reserved field. Deliberately not claimed yet: user/admin trust policy, persisted grant records, runtime enforcement of undeclared privileged operations, ABT registration, symbol dispatch, integrity, diagnostics, fuzzing, broader QEMU validation, or physical hardware validation. Phases 10-15 remain real, separate, later increments.|
|0.9|2026-08-24|Section 61 Phase 10 (ArcoBASIC Binding Table registration, Section 20/21) real and QEMU-proven. Concrete 56-byte fixed ARCO_BINDING content layout fixed for the first ABT registration slice: BindingID, OwningComponentID, BindingKind (OBJECT/PROPERTY/METHOD/EVENT), optional TargetImplementationID, 128-bit NameID, 128-bit AuthorityID, Flags, and zero Reserved field. `AEX.RegisterArcoBindings` (`stdlib/aex_format_policy.abas`) walks ARCO_BINDING chunks from the validated directory, validates bounds, chunk/content ID agreement, manifest-declared owning component, legal binding kind, reserved-field cleanliness, and optional target implementation type/owner, then copies accepted bindings into a bounded loader ABT scratch registry (`AexBindingRegistry*`) for later shell/inspector/runtime handoff. Proven by new smoke test `systems_aex_arco_binding_smoke`: a byte-exact fixture registers one METHOD binding targeting a native implementation and covers too-short binding, undeclared owner, invalid kind, binding-ID mismatch, missing target implementation, target-owner mismatch, and nonzero reserved field. Deliberately not claimed yet: wrapper generation, object instance exposure, property/method dispatch, authority enforcement for ABT calls, integrity, diagnostics, fuzzing, broader QEMU validation, or physical hardware validation. Phases 11-15 remain real, separate, later increments.|
|1.0|2026-08-24|Section 61 Phase 11 (first-stage integrity verification, Section 26/27 and Loader Stage 7) real and QEMU-proven. Concrete 32-byte fixed INTEGRITY content layout fixed for this initial slice: IntegrityID, Algorithm, CoveredChunkID, ExpectedChecksum32, CoveredLength, Flags, and zero Reserved field. `AEX.VerifyIntegrity` (`stdlib/aex_format_policy.abas`) validates every INTEGRITY chunk, computes the current bootstrap `AEX_CHECKSUM32` over covered chunk bytes, rejects checksum mismatches, malformed records, unsupported algorithms, missing covered chunks, length mismatches, and missing integrity coverage for CODE chunks before activation. This is explicitly not claimed as cryptographic SHA-256 integrity: `AEX_CHECKSUM32` is a deterministic fail-closed bootstrap gate until the freestanding substrate exposes a real cryptographic primitive. Proven by new smoke test `systems_aex_integrity_verification_smoke`: a byte-exact fixture protects one CODE chunk and covers valid verification, code-byte tampering, missing INTEGRITY record, too-short integrity record, unsupported algorithm, missing covered chunk, and covered-length mismatch. Deliberately not claimed yet: SHA-256 or stronger digest verification, integrity trees over all required metadata, signatures, diagnostics, fuzzing, broader QEMU validation, or physical hardware validation. Phases 12-15 remain real, separate, later increments.|
|1.1|2026-08-24|Section 61 Phase 12 (diagnostic metadata registration, Section 29/38) real and QEMU-proven. Concrete 76-byte fixed DIAGNOSTIC content layout fixed for this attribution slice: DiagnosticID, OwningComponentID, DiagnosticKind, optional TargetImplementationID, optional CodeChunkID plus CodeOffset/CodeLength range, 128-bit NameID, 128-bit DomainID, Flags, and zero Reserved field. `AEX.RegisterDiagnostics` (`stdlib/aex_format_policy.abas`) validates every DIAGNOSTIC chunk, confirms declared component ownership, legal kind, target implementation ownership, CODE chunk type and range bounds, reserved-field cleanliness, and then copies accepted records into a bounded loader diagnostics scratch registry for later loader/trap/shell fault reporting. Proven by new smoke test `systems_aex_diagnostics_metadata_smoke`: a byte-exact fixture registers one operation diagnostic record and covers too-short metadata, ID mismatch, undeclared owner, invalid kind, missing target implementation, invalid code range, and full-registry rejection. Deliberately not claimed yet: source-level symbols, text-string interning, trap-frame integration, user-facing diagnostic rendering, fuzzing, broader QEMU validation, or physical hardware validation. Phases 13-15 remain real, separate, later increments.|
|1.2|2026-08-24|Section 61 Phase 13 (negative fixtures and deterministic fuzz-style mutation matrix, Section 56/57) real and QEMU-proven. Added `systems_aex_negative_matrix_smoke`, which builds one valid byte-exact pre-activation AEX image and then mutates it in-place across the implemented loader surfaces: bad magic, bad header size, directory bounds, unknown required chunk type, chunk bounds, missing manifest, unsupported implementation architecture, integrity mismatch, malformed ABT binding, and malformed diagnostic range. The same fixture first proves the unmodified image passes the implemented metadata pipeline through header, directory, manifest, native implementation selection, integrity verification, ABT registration, and diagnostic registration. This is deterministic malformed-input regression coverage, not a coverage-guided random fuzzer. Deliberately not claimed yet: persistent fuzz corpus management, sanitizer-backed host fuzzing, broader QEMU validation beyond the AEX smoke suite, or physical hardware validation. Phases 14-15 remain real, separate, later increments.|
|1.3|2026-08-24|Section 61 Phase 14 (combined automated QEMU validation, Section 58) real and QEMU-proven. Added `systems_aex_qemu_load_path_smoke`, a single end-to-end substrate path that creates a byte-exact AEX image, writes it through ArcFS, reloads it with `AEX.LoadFromArcFS`, validates header/directory/manifest, selects the native implementation, verifies CODE integrity, registers ABT and diagnostic metadata, maps CODE and DATA, resolves lifecycle metadata, and invokes the native lifecycle entry under QEMU with the expected return value. This consolidates the previously isolated feature smokes into one automated load-path proof. Deliberately not claimed yet: allocator-backed executable mappings, W^X page-table enforcement, multi-argument ABI calls, signatures, host-side corpus fuzzing, or physical hardware validation. Phase 15 remains a separate later increment.|
|1.4|2026-08-24|Added the first substrate loader orchestration entry point beyond the Section 61 checklist: `AEX.LoadAndActivate0`. It composes the implemented no-argument native activation pipeline into one call: ArcFS load, header/directory/manifest validation, root implementation selection, integrity verification, ABT registration, diagnostic registration, CODE/DATA mapping, lifecycle resolution, and native invocation. It records the last failing stage, error code, loaded file length, and native return value in scratch cells so callers can distinguish where the loader stopped. Proven by new smoke test `systems_aex_load_and_activate_smoke`, covering successful activation, missing ArcFS object failure, and CODE destination-capacity failure. Deliberately not claimed yet: allocator-backed executable mappings, W^X enforcement, argument/context passing, unload/deactivation, signatures, or physical hardware validation.|
|1.5|2026-08-24|Added the first real MEMORY_POLICY metadata consumer for Section 17. Concrete 28-byte fixed MEMORY_POLICY content layout now validates MemoryPolicyID, owning component, optional target implementation, optional CODE/DATA target chunk, permission flags, semantic flags, zero reserved field, immutable/write contradictions, private/shared contradictions, ZERO_INITIALIZED target rules, and default W^X denial. `AEX.LoadAndActivate0` now runs `AEX.ValidateMemoryPolicies` after implementation selection and before integrity/mapping. Proven by new smoke test `systems_aex_memory_policy_smoke`, covering valid CODE/DATA policies, too-short policy chunks, ID mismatch, undeclared owner, write+execute rejection, invalid target chunk, and immutable+write rejection. Deliberately not claimed yet: page-table permission programming, allocator-backed mappings, copy-on-write runtime machinery, or persisted memory-region descriptors.|
|1.6|2026-08-24|Added the first allocator-backed mapping and native ABI expansion slice. `AEX.AllocateAndMapSelectedCode` and `AEX.AllocateAndMapSelectedData` allocate page-backed destination regions through UEFI `BootServices.AllocatePages`, copy the selected implementation's CODE/DATA chunks into those allocated pages, and publish allocated addresses/capacities in loader scratch cells. The compiler/frontend now recognize `AEX.InvokeNative1(entryAddress, arg0)` and lower it to the Microsoft x64 first integer argument register (`RCX`) followed by an indirect call through the resolved native entry. `AEX.InvokeResolvedLifecycle1` wraps resolved lifecycle offsets for one-argument calls, and `AEX.LoadAndActivateAllocated1` composes ArcFS load, metadata validation, memory-policy validation, integrity, ABT/diagnostic registration, allocated mapping, lifecycle resolution, and one-argument native invocation. Proven by new smoke test `systems_aex_allocated_native1_smoke`, where allocated CODE bytes execute `return arg0 + 7` and return 42 under QEMU. Deliberately not claimed yet: page-table NX/RW permission programming after allocation, multi-argument/context ABI, data-pointer ABI passing, deactivation/unload, signatures, or physical hardware validation.|
|1.7|2026-08-24|Added the next two AEX substrate functionality slices. `AEX.ResolveInterfaces` now validates declared interface metadata and materializes required-to-provided interface bindings into a bounded loader scratch registry, matching on 128-bit contract ID and compatible major/minor versions while rejecting missing providers and malformed provider ownership. The Phase 8 QEMU fixture now proves a successful resolution entry and a metadata-valid missing-provider rejection. Capability evaluation now has explicit grant-state tracking: `AEX.ClearCapabilityGrants`, `AEX.AllowCapability`, `AEX.CapabilityGrantCount`, `AEX.CapabilityIsGranted`, and `AEX.EvaluateCapabilityRequestsWithGrants` validate request metadata separately from authorization and permit required capabilities only when the exact 128-bit capability ID has been granted. The legacy `AEX.EvaluateCapabilityRequests` remains fail-closed for required capabilities. The Phase 9 QEMU fixture now proves granted-required acceptance and wrong-grant denial. Deliberately not claimed yet: persisted trust policy, UI/admin grant sources, runtime privileged-operation enforcement, interface dispatch stubs, or version-range negotiation beyond the fixed major/minor fields.|
|1.8|2026-08-24|Wired the new interface and capability gates into the activation substrate and added the first resolved-interface-to-ABT lookup surface. `AEX.LoadAndActivate0` and `AEX.LoadAndActivateAllocated1` now run `AEX.ResolveInterfaces` and `AEX.EvaluateCapabilityRequestsWithGrants` after manifest validation and before implementation selection, recording distinct loader stages for interface-resolution and capability-authorization failures. The ABT registry now exposes bounded public readers plus `AEX.FindResolvedProviderBinding(requiredInterfaceID, bindingKind, nameIDHigh, nameIDLow)`, which follows the resolved interface table to the provider component and returns a matching provider-owned binding ID for later dispatch stubs. The Phase 8 QEMU fixture now includes a provider binding and proves successful lookup, wrong-kind miss, and unresolved-interface miss; activation smokes still pass through the stricter gates. Deliberately not claimed yet: generated dispatch stubs, method invocation through ABT records, persisted/user-sourced grants, or runtime operation enforcement.|
|1.9|2026-08-24|Added the next two AEX dispatch/authority slices. `AEX.RegisterArcoBindingsWithAuthority` now validates ABT records and only registers bindings whose nonzero AuthorityID is present in the explicit capability grant table; zero AuthorityID remains public/unrestricted. `AEX.LoadAndActivate0` and `AEX.LoadAndActivateAllocated1` now use the authority-aware ABT registration path, so activation fails before mapping/invocation if exposed bindings require missing grants. The Phase 10 QEMU fixture now proves denied and granted authority registration. The diagnostics/dispatch side now exposes diagnostic registry readers plus `AEX.FindBindingDispatchDiagnostic`, `AEX.FindBindingDispatchCodeChunk`, and `AEX.FindBindingDispatchCodeOffset`, mapping a registered METHOD binding to matching operation diagnostic metadata with the same owner, target implementation, and 128-bit name. The Phase 12 QEMU fixture now proves dispatch diagnostic/code lookup and wrong-kind miss behavior. Deliberately not claimed yet: generated dispatch stubs, indirect method invocation through dispatch records, persisted/user-sourced grants, or runtime operation enforcement beyond registration-time authority filtering.|
|2.0|2026-08-24|Added the final two current AEX functionality slices. Dispatch metadata now has an executable consumer: `AEX.InvokeBindingDispatch1(bindingID, codeBase, arg0)` resolves a registered METHOD binding through diagnostic dispatch metadata and invokes the mapped native code offset with the existing one-argument native ABI. The allocator-backed native smoke now registers binding/diagnostic dispatch metadata and proves a binding dispatch call returns the expected value under QEMU. LIFECYCLE records are now backward-compatible at 16 bytes for activation and may extend to 24 bytes with `DeactivateCodeOffset`; `AEX.ResolveLifecycleEntry` validates and records that optional deactivate entry, and `AEX.InvokeResolvedLifecycleDeactivate0` invokes it. The lifecycle QEMU fixture now maps both activate and deactivate native entries and proves both execute. Deliberately not claimed yet: generated high-level ArcoBASIC wrapper stubs, unload resource teardown policy, persisted/user-sourced grants, runtime operation enforcement beyond registration-time authority filtering, or page-table NX/RW permission programming.|
