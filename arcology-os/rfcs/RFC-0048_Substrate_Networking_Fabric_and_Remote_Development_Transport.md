...
Status: Draft -- Milestone N0 (NIC Transport) fully implemented and QEMU-proven: real PCI
discovery, a generic Network Interface Contract, real VirtIO-net legacy PCI transport (I/O-BAR
mapping, reset, feature negotiation, MAC/link-state readback), and real virtqueue-based frame
transmit/receive with explicit ownership semantics -- both directions proven against a real QEMU
virtio-net-pci device via independent off-the-wire capture and injection. Steered by
RFC-0048-Steering-01 (2026-08-26) -- see that document for the revised upper-layer direction
(Arcology Interconnect supersedes the original disposable ARCODEV/1 milestone starting at N5).
Milestones N1-N9 not started
...
|0.4|2026-08-26|Milestone N0 (NIC Transport, Section 18.1) fully completed: real virtqueue setup and real frame transmit/receive with explicit ownership semantics (Section 6.1; Steering Addendum Section 2.2). `VirtioNet.SetupQueue` reads the real hardware-reported QueueSize for each of the two legacy virtio-net queues (receiveq=0, transmitq=1; VIRTIO_NET_F_CTRL_VQ not negotiated, so no third queue), computes the real legacy virtqueue layout (descriptor table, avail ring, page-aligned used ring) from that real value rather than a hardcoded size, and allocates/zeroes/programs it via the same `BootServices.AllocatePages` idiom this project's other drivers already established. `VirtioNet.PostRxBuffer`/`VirtioNet.TransmitFrame` publish descriptors to the avail ring (an explicit, code-visible ownership transfer to the device, documented at every transfer point) and ring the real per-queue doorbell; `VirtioNet.TryConsumeUsed` is the single non-blocking check where ownership returns to the driver, comparing the real hardware used-ring index against a 16-bit-narrowed local counter so free-running-counter wraparound compares correctly. `NetworkInterface.SetupReceive/SetupTransmit/Transmit/PollTransmitComplete/PollReceive/ReceiveBufferAddress/ReceiveHeaderSize` extend the generic contract to cover the full data path -- `ReceiveHeaderSize` in particular reports VirtIO-net's own real 10-byte `virtio_net_hdr` length as backend-reported data rather than a caller-hardcoded constant, so a caller never needs backend-specific wire-format knowledge. Proven end-to-end by the extended `systems_virtio_net_transport_smoke`, using a new `arcology-os/scripts/run/virtio_net_frame_tool.py` helper against QEMU's `-netdev socket,udp=...,localaddr=...` backend (a plain one-frame-per-UDP-datagram tunnel): the fixture's own transmitted 60-byte frame is captured completely independently, off the wire, and matched byte-for-byte; a known 60-byte frame is injected from outside the guest only after the fixture's own real `RXREADY` marker appears (matching this project's own established "wait for a readiness marker, not just socket existence" injection discipline), and the fixture correctly receives it, skips the real device-written header, and reports its real parsed content (length, destination/source MAC, EtherType, first 8 payload bytes) byte-for-byte over serial. Both directions passed on the first real end-to-end run, and the RX-injection race (harness must bind its own capture listener before QEMU can possibly transmit) was handled correctly the first time by starting that listener before QEMU itself. Main's own local-variable count required splitting the proof into several helper functions (`BuildTxFrame`, `RunTxAndReport`, `ReportRxFrame`, `RunRxPollAndReport`, `RunQueueProof`) after hitting this backend's real stack-frame-size ceiling -- a real, previously-undocumented-in-this-file compiler constraint, not a logic bug. Scoped deliberately to one descriptor/one buffer per queue (no buffer pooling, no chained descriptors, no interrupts -- polling only); a production driver would pool multiple buffers per queue without changing the ownership discipline documented here. This completes RFC-0048's Milestone N0 in full. Milestones N1-N9 remain real, separate, later increments.|
