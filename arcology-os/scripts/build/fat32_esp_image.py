"""Shared, deterministic FAT32 filesystem builder (RFC-0044 Section 5, point 3).

Extracted verbatim from build-arcology-hardware-image.py's own original `build_image` function
(no behavioral change -- confirmed byte-identical output before this refactor was ever committed,
see the header note in build-arcology-hardware-image.py itself) so it can be reused by
build-arcology-gpt-image.py to build the ESP partition's own filesystem bytes, without duplicating
this logic a second time.

All geometry, identifiers, allocation order, directory timestamps, and unused bytes are fixed --
the same file, given the same EFI application bytes and the same TOTAL_SECTORS, always produces
the same output. The addressing inside these bytes is entirely relative to the START of this FAT32
volume itself (standard FAT32 behavior) -- a caller embedding this as a GPT partition only needs to
place these bytes starting at that partition's own first LBA; nothing here needs to know or care
what absolute disk offset it ends up at.
"""
...
SECTOR_SIZE = 512
DEFAULT_TOTAL_SECTORS = 131_072  # 64 MiB -- build-arcology-hardware-image.py's own original size
RESERVED_SECTORS = 32
FAT_COUNT = 2
...
def build_fat32_image(efi: bytes, total_sectors: int = DEFAULT_TOTAL_SECTORS) -> bytes:
    if not efi:
        raise ValueError("EFI application is empty")

    fat_sectors = sectors_per_fat(total_sectors)
    data_start = RESERVED_SECTORS + FAT_COUNT * fat_sectors
    data_sectors = total_sectors - data_start
    cluster_count = data_sectors // SECTORS_PER_CLUSTER
    if cluster_count < 65_525:
        raise AssertionError("requested geometry no longer qualifies as FAT32")
    ...
    if file_last_cluster >= cluster_count + 2:
        raise ValueError("EFI application does not fit in the requested image size")
