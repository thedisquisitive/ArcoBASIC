...
// General value equality (RFC-0053, "full Linux support" pass -- found genuinely blocking Arconaut,
...
// Ordering for "<"/"<="/">"/">=" (found missing entirely -- ArcoSH WP-006's own
// `ch >= "0" ANDALSO ch <= "9"`-shaped digit check panicked with "value is not a number" on the
// interpreter and failed native compilation with "unsupported operation ... on this backend",
// since every relational-operator codegen path unconditionally treated both operands as numbers;
// this is a LANGUAGE-level gap, not something specific to this backend, so the fix lives in
// arco::values_compare (include/arco/value.hpp), which this just calls). Mirrors it exactly: if
// either operand is a string, both sides are stringified and compared lexicographically;
// otherwise both must coerce to a number (bools included) or this panics, matching
// arco_value_as_number's own "value is not a number" panic for a genuinely non-orderable value
// (arrays/objects/handles). Returns -1/0/1, not a bool -- the generated code compares the result
// against zero with the operator-appropriate signed condition (L/LE/G/GE) so one ABI call backs
// all four operators.
int arco_value_compare(const ArcoValue* left, const ArcoValue* right);
...
// generated Binary/Unary arithmetic to unbox a Boxed array element/object field (Phase 2, RFC-0053
// Section 4) before doing real SSE2 math on it.
double arco_value_as_number(const ArcoValue* value);
// Never panics -- mirrors arco::Value::truthy() exactly (null/empty-string/empty-array/0 are
// falsy, everything else truthy), used by generated Unary "!" to unbox a Boxed operand (an object
// field/array element/host-function-call result whose value happens to be a bool, e.g. `!line.Ok`)
// before negating. A real, disclosed miscompilation this closes: the generic Unary "!" path used
// to load a Boxed operand's raw ArcoValue* pointer into RAX and mask/compare ITS bits directly
// (as though it were already an unboxed 0/1 bool) instead of dereferencing the box at all --
// `(pointer & 0xff) == 0` on a real heap pointer is essentially a coin flip depending on the
// allocator's own address, not the value stored inside, so this "worked" only by heap-layout
// coincidence in the smallest repros and broke the instant a second host call anywhere in the same
// loop shifted the allocation pattern. See RFC-0053's own progress notes, Entry 25/Bug C.
int arco_value_truthy(const ArcoValue* value);
...
// --- Arrays and objects (Phase 2 -- RFC-0053 Section 4): the actual reason ArcoValue exists
...
// Kind::Return codegen, RFC-0053 Phase 5, .agents/ARCO_NATIVE_RUNTIME_PROGRESS.md Entry 14): this
...
// Tuples (RFC-0053, "full Linux support" pass): unlike an array (built incrementally via
...
// Index-by-number (RFC-0053, "full Linux support" pass): a general-purpose sibling to
...
// Runtime.Args() (RFC-0053, "full Linux support" pass): the process's own command-line arguments.
...
// Script-scope globals (RFC-0053 Section 4, the mechanism apply_script_global_scoping's own
...
// TRY/CATCH (RFC-0053): generated native code has no unwind tables for a C++-style exception to
