...
constexpr std::uint64_t kMaxSnapshots = 4096;
constexpr std::uint64_t kSnapshotLabelBytes = 56;
constexpr std::uint64_t kChunkSize = 4096;
constexpr std::uint64_t kIncompatMultiSectorBitmap = 1ULL << 16;
// Recognized-but-optional: unlike kIncompatMultiSectorBitmap (required on every volume this
// reader mounts, see select_superblock()'s own hard check), a volume with no snapshots yet never
// needs this bit set, so an old 112-byte-checkpoint volume keeps mounting cleanly forever. Once
// this build commits anything at all, it always writes the extended 128-byte checkpoint format
// (see checkpoint_record()/Volume::commit()) and sets this bit -- a one-way upgrade the moment
// the volume is written to, so an older host.cpp binary correctly refuses to mount it afterward
// (select_superblock()) instead of silently mishandling the fields it doesn't know about.
constexpr std::uint64_t kIncompatSnapshotTree = 1ULL << 17;
// select_superblock() checks this against `incompat`, which is ALREADY right-shifted 16 bits
// ((raw_flags >> 16) & 0xFFFF) -- so its own bits are relative to that submask (bit N here means
// raw_flags bit 16+N), NOT the same representation as the two absolute-raw-bit-position constants
// above (kIncompatMultiSectorBitmap/kIncompatSnapshotTree, used directly against raw_flags by the
// hard "must be present" check and when OR'd into a written feature_flags value). Bit 0 here =
// kIncompatMultiSectorBitmap (raw bit 16); bit 1 = kIncompatSnapshotTree (raw bit 17). ORing the
// raw constants in directly here (as an earlier version of this line did) is a real bug: their raw
// bit-16/17 values fall entirely outside this 16-bit submask's own 0-15 range and contribute
// nothing, silently un-recognizing kIncompatSnapshotTree and rejecting every volume that sets it.
constexpr std::uint64_t kRecognizedIncompat = 0x3;
constexpr std::uint64_t kRecognizedRoCompat = 0;
// Checkpoint record length once SnapshotTreeRoot/SnapshotTreeCount are included (RFC-0042 section
// 8's self-describing-length mechanism -- see read_checkpoint()). Was 112 before this field pair.
constexpr std::uint64_t kCheckpointRecordLength = 128;
...
std::vector<std::uint8_t> checkpoint_record(const FormatLayout& layout) {
    ...
    put64(record, 88, 0);
    put64(record, 96, 0);
    put64(record, 104, kCheckpointRecordLength);
    put64(record, 112, 0); // snapshot_tree_root: a freshly formatted volume has no snapshots
    put64(record, 120, 0); // snapshot_tree_count
    put32(record, static_cast<std::size_t>(kCheckpointRecordLength),
         crc32c(record.data(), static_cast<std::size_t>(kCheckpointRecordLength)));
    return record;
}
...
std::vector<std::uint8_t> superblock_record(const FormatLayout& layout, std::uint64_t volume_id) {
    ...
    // Called both at mkfs time and on every commit() -- every volume this build touches at all
    // (formatted fresh, or merely written to once) ends up with kIncompatSnapshotTree set, which
    // is exactly the intended one-way format upgrade (see that constant's own comment above).
    put64(record, 48, kIncompatMultiSectorBitmap | kIncompatSnapshotTree);
...
// A snapshot label is a fixed-width NUL-padded field (no separate length prefix, unlike namespace
// names above) -- reads up to the first NUL within the field, or the whole field if unterminated.
std::string read_fixed_label(const std::vector<std::uint8_t>& bytes, std::size_t offset, std::size_t width) {
    require(offset + width <= bytes.size(), "ArcFS snapshot label exceeds source buffer");
    std::size_t length = 0;
    while (length < width && bytes[offset + length] != 0) ++length;
    return std::string(reinterpret_cast<const char*>(bytes.data() + offset), length);
}

// Truncates to fit -- a label is cosmetic, not an identifier (snapshot_id is), so silent
// truncation of an overlong label is an acceptable, documented (host.hpp) limit rather than a
// hard error the way a corrupt/invalid on-disk name is elsewhere in this file.
void write_fixed_label(std::vector<std::uint8_t>& row, std::size_t offset, std::size_t width, const std::string& label) {
    const auto take = std::min(width, label.size());
    std::copy(label.begin(), label.begin() + static_cast<std::ptrdiff_t>(take), row.begin() + static_cast<std::ptrdiff_t>(offset));
}
...
Superblock Volume::select_superblock() const {
    ...
    for (std::uint64_t slot = 0; slot < 4; ++slot) {
        ...
        try {
            ...
            // Malformed checkpoint at this slot -- keep scanning the other ring entries.
        }
    }
...
Checkpoint Volume::read_checkpoint(std::uint64_t sector) const {
    ...
    cp.previous_sector = le64(data, 48);
    // Only present in the extended (>=128-byte) checkpoint record; an older, still-valid 112-byte
    // checkpoint has no snapshots at all, decoded as 0/0 rather than an error -- see
    // kIncompatSnapshotTree's own comment for why this is safe/expected for pre-existing volumes.
    if (payload_len >= kCheckpointRecordLength) {
        cp.snapshot_tree_root = le64(data, 112);
        cp.snapshot_tree_count = le64(data, 120);
    }
    ...
    require(cp.snapshot_tree_count <= kMaxSnapshots, "ArcFS snapshot count exceeds host reader limit");
    return cp;
}
...
// Parses one checkpoint's Object/Namespace/Extent/Attribute trees into a standalone, read-only
// view -- the same parsing mount() itself needs for the live checkpoint_, factored out so both
// the live mount path and every *_in_snapshot accessor (which needs this for an arbitrary past
// checkpoint) share one implementation instead of two copies that can silently drift apart (see
// the attribute-tree commit() bug found via exactly that kind of drift, above). Uses superblock_/
// source_/byte_offset_ of the currently-mounted volume -- valid for any checkpoint reachable on
// that same physical volume, live or historical.
Volume::TreeView Volume::load_tree_view(const Checkpoint& source) const {
    TreeView view;

    const auto object_entries = collect_tree(source.object_root, source.object_count, 40, kMaxObjects);
    for (std::size_t o = 0; o < object_entries.size(); o += 40) {
        ...
        view.objects[obj.oid] = obj;
    }
    require(view.objects.count(1) != 0 && view.objects.at(1).type == 2, "ArcFS root directory is missing");

    const auto ns_entries = collect_tree(source.namespace_root, source.namespace_count, 56, kMaxNamespaceRows);
    for (std::size_t o = 0; o < ns_entries.size(); o += 56) {
        ...
        require(view.objects.count(entry.parent_oid) != 0 && view.objects.count(entry.child_oid) != 0,
                "ArcFS namespace entry references a missing object");
        view.namespace_entries.push_back(std::move(entry));
    }

    std::uint64_t max_referenced_extents = 0;
    for (const auto& [_, obj] : view.objects) {
    ...
    require(source.extent_tree_count <= max_referenced_extents, "ArcFS extent count exceeds referenced file data");
    require(source.extent_tree_count <= max_addressable_extents, "ArcFS extent count exceeds volume capacity");
    const auto extent_entries = collect_tree(source.extent_tree_root, source.extent_tree_count, 32, max_referenced_extents);
    for (std::size_t o = 0; o < extent_entries.size(); o += 32) {
        ...
        view.extents[{data_slot, chunk}] = sector;
    }

    const auto attr_entries = collect_tree(source.attribute_tree_root, source.attribute_tree_count, 48, kMaxAttributes);
    for (std::size_t o = 0; o < attr_entries.size(); o += 48) {
        ...
        require(view.objects.count(attr.oid) != 0, "ArcFS attribute references a missing object");
        view.attributes.push_back(attr);
    }
...
void Volume::mount(std::shared_ptr<BlockSource> source, MountOptions options) {
    ...
    auto view = load_tree_view(checkpoint_);
    objects_ = std::move(view.objects);
    namespace_entries_ = std::move(view.namespace_entries);
    extents_ = std::move(view.extents);
    attributes_ = std::move(view.attributes);
    ...
    const auto snapshot_entries = collect_tree(checkpoint_.snapshot_tree_root, checkpoint_.snapshot_tree_count, 96, kMaxSnapshots);
    for (std::size_t o = 0; o < snapshot_entries.size(); o += 96) {
        SnapshotRecord snap;
        snap.snapshot_id = le64(snapshot_entries, o);
        snap.source_generation = le64(snapshot_entries, o + 8);
        snap.checkpoint_sector = le64(snapshot_entries, o + 16);
        snap.created_at = le64(snapshot_entries, o + 24);
        snap.flags = le64(snapshot_entries, o + 32);
        snap.label = read_fixed_label(snapshot_entries, o + 40, kSnapshotLabelBytes);
        require(snap.snapshot_id != 0 && snap.checkpoint_sector != 0, "ArcFS snapshot entry is invalid");
        snapshots_.push_back(std::move(snap));
        if (snap.snapshot_id >= next_snapshot_id_) next_snapshot_id_ = snap.snapshot_id + 1;
    }
}
...
void Volume::commit(bool sync) {
    ...
    // Was hardcoded to always discard attributes_ on every commit (attr_root/count always 0) --
    // a real, dormant data-loss bug found while adding the Snapshot Tree below (same rebuild-on-
    // every-commit pattern object_rows/namespace_rows already use). Currently harmless in
    // practice since no write path populates attributes_ yet, but a real landmine for whatever
    // does first.
    std::vector<std::vector<std::uint8_t>> attribute_rows;
    for (const auto& attr : attributes_) {
        std::vector<std::uint8_t> row(48, 0);
        put64(row, 0, attr.oid);
        put64(row, 8, attr.oid_high);
        put64(row, 16, attr.id);
        put64(row, 24, attr.type);
        put64(row, 32, attr.value_low);
        put64(row, 40, attr.value_high);
        attribute_rows.push_back(std::move(row));
    }
    std::sort(attribute_rows.begin(), attribute_rows.end(), [](const auto& a, const auto& b) {
        if (le64(a, 0) != le64(b, 0)) return le64(a, 0) < le64(b, 0);
        return le64(a, 16) < le64(b, 16);
    });

    std::vector<std::vector<std::uint8_t>> snapshot_rows;
    for (const auto& snap : snapshots_) {
        std::vector<std::uint8_t> row(96, 0);
        put64(row, 0, snap.snapshot_id);
        put64(row, 8, snap.source_generation);
        put64(row, 16, snap.checkpoint_sector);
        put64(row, 24, snap.created_at);
        put64(row, 32, snap.flags);
        write_fixed_label(row, 40, kSnapshotLabelBytes, snap.label);
        snapshot_rows.push_back(std::move(row));
    }
    std::sort(snapshot_rows.begin(), snapshot_rows.end(), [](const auto& a, const auto& b) { return le64(a, 0) < le64(b, 0); });
    ...
    const auto attr_root = write_tree(attribute_rows, 48);
    const auto snapshot_root = write_tree(snapshot_rows, 96);
    ...
    put64(checkpoint, 80, attribute_rows.size());
    put64(checkpoint, 88, extent_root);
    put64(checkpoint, 96, extent_entries.size());
    put64(checkpoint, 104, kCheckpointRecordLength);
    put64(checkpoint, 112, snapshot_root);
    put64(checkpoint, 120, snapshot_rows.size());
    put32(checkpoint, static_cast<std::size_t>(kCheckpointRecordLength),
         crc32c(checkpoint.data(), static_cast<std::size_t>(kCheckpointRecordLength)));
...
const SnapshotRecord* Volume::find_snapshot(std::uint64_t snapshot_id) const {
    for (const auto& snap : snapshots_) {
...
std::uint64_t Volume::create_snapshot(const std::string& label) {
    require(!metadata_dirty_ && dirty_files_.empty(), "ArcFS snapshot requires a clean volume -- commit pending changes first");
    require(label.size() <= kSnapshotLabelBytes, "ArcFS snapshot label exceeds host reader limit");
    SnapshotRecord snap;
    snap.snapshot_id = next_snapshot_id_++;
    snap.source_generation = checkpoint_.generation;
    snap.checkpoint_sector = checkpoint_.sector;
    snap.created_at = static_cast<std::uint64_t>(
        std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count());
    snap.flags = 0;
    snap.label = label;
    const auto id = snap.snapshot_id;
    snapshots_.push_back(std::move(snap));
    // Nothing else is dirty (checked above), but the Snapshot Tree row is -- force commit() past
    // its own "nothing to do" no-op so the new row actually gets written and durably reachable.
    metadata_dirty_ = true;
    commit();
    return id;
}

bool Volume::delete_snapshot(std::uint64_t snapshot_id) {
    const auto it = std::find_if(snapshots_.begin(), snapshots_.end(),
                                 [&](const auto& s) { return s.snapshot_id == snapshot_id; });
    if (it == snapshots_.end()) return false;
    snapshots_.erase(it);
    // Removes the Snapshot Tree row only -- see host.hpp's own comment on this method for why no
    // space is freed (host.cpp has no reclamation machinery at all yet, snapshot or not).
    metadata_dirty_ = true;
    commit();
    return true;
}

void Volume::rollback_to_snapshot(std::uint64_t snapshot_id) {
    const auto* snap = find_snapshot(snapshot_id);
    require(snap != nullptr, "ArcFS snapshot does not exist");
    require(!metadata_dirty_ && dirty_files_.empty(), "ArcFS rollback requires a clean volume -- commit or discard pending changes first");
    const auto old_checkpoint = read_checkpoint(snap->checkpoint_sector);
    auto view = load_tree_view(old_checkpoint);
    objects_ = std::move(view.objects);
    namespace_entries_ = std::move(view.namespace_entries);
    extents_ = std::move(view.extents);
    attributes_ = std::move(view.attributes);
    ...
    // The Snapshot Tree itself (including the snapshot just restored from) is untouched -- commit()
    // rebuilds it verbatim from the unmodified snapshots_ vector, same as every other commit.
    metadata_dirty_ = true;
    commit();
}

std::optional<std::uint64_t> Volume::resolve_posix_path_in_snapshot(std::uint64_t snapshot_id, const std::string& path) const {
    const auto* snap = find_snapshot(snapshot_id);
    require(snap != nullptr, "ArcFS snapshot does not exist");
    const auto view = load_tree_view(read_checkpoint(snap->checkpoint_sector));
    std::uint64_t current = 1;
    for (const auto& part : split_posix_path(path)) {
        bool found = false;
        for (const auto& entry : view.namespace_entries) {
            if (entry.parent_oid == current && entry.name == part) {
                current = entry.child_oid;
                found = true;
                break;
...
std::optional<Object> Volume::object_in_snapshot(std::uint64_t snapshot_id, std::uint64_t oid) const {
    const auto* snap = find_snapshot(snapshot_id);
    require(snap != nullptr, "ArcFS snapshot does not exist");
    const auto view = load_tree_view(read_checkpoint(snap->checkpoint_sector));
    const auto it = view.objects.find(oid);
    if (it == view.objects.end()) return std::nullopt;
    return it->second;
}

std::vector<DirectoryEntry> Volume::children_in_snapshot(std::uint64_t snapshot_id, std::uint64_t parent_oid) const {
    const auto* snap = find_snapshot(snapshot_id);
    require(snap != nullptr, "ArcFS snapshot does not exist");
    const auto view = load_tree_view(read_checkpoint(snap->checkpoint_sector));
    std::vector<DirectoryEntry> result;
...
std::vector<std::uint8_t> Volume::read_file_in_snapshot(std::uint64_t snapshot_id, std::uint64_t oid,
                                                        std::uint64_t offset, std::size_t size) const {
    const auto* snap = find_snapshot(snapshot_id);
    require(snap != nullptr, "ArcFS snapshot does not exist");
    const auto view = load_tree_view(read_checkpoint(snap->checkpoint_sector));
    const auto obj_it = view.objects.find(oid);
    require(obj_it != view.objects.end() && obj_it->second.type == 1, "ArcFS object is not a file");
    ...
    while (written < to_read) {
        ...
        const auto in_chunk = logical % kChunkSize;
        const auto take = static_cast<std::size_t>(std::min<std::uint64_t>(kChunkSize - in_chunk, to_read - written));
        const auto extent = view.extents.find({obj.data_slot, chunk});
        if (extent != view.extents.end()) {
            const auto data = source_->read_at(byte_offset_ + extent->second * kSectorSize + in_chunk, take);
            std::copy(data.begin(), data.end(), out.begin() + static_cast<std::ptrdiff_t>(written));
        }
        logical += take;
