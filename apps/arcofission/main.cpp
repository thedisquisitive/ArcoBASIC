...
void print_usage(std::ostream& output) {
    output
        ...
        << "  ArcoFission build FILE -o OUT --target linux-x86_64 [--entry NAME] [--debug] [--sanitize]\n"
        ...
        << "    --debug writes OUT.s, the generated assembly annotated with a `# [Function] ...`\n"
        << "    comment -- the exact same rendering `reveal amir` produces -- immediately before\n"
        << "    each AMIR instruction's own generated bytes, so a raw address from gdb/objdump maps\n"
        << "    straight back to source. --sanitize adds -fsanitize=address plus real debug symbols\n"
        << "    to the underlying compile. Both are debugging aids with real cost -- larger output/\n"
        << "    slower binary -- never implied by a plain build.)\n"
...
int main(int argc, char** argv) {
    ...
    if (argc >= 5 && lowercase(argv[1]) == "build" && std::string(argv[3]) == "-o") {
        ...
        std::string entry_function = "Main";
        std::optional<std::size_t> instruction_limit = default_unlimited_instruction_limit();
        // --debug/--sanitize (the Arco native debugger tooling, --target linux-x86_64 only -- see
        // print_usage's own comment) are plain boolean flags, no value -- everything else here is a
        // "--flag value" pair, so those two are checked first and skip only ONE argv slot, not two.
        arco::fission::NativeDebugOptions native_debug_options;
        for (int i = 5; i < argc; ) {
            const std::string flag = lowercase(argv[i]);
            if (flag == "--debug") {
                native_debug_options.annotate = true;
                i += 1;
                continue;
            }
            if (flag == "--sanitize") {
                native_debug_options.sanitize = true;
                i += 1;
                continue;
            }
            if (i + 1 >= argc) break;
            ...
            } else if (flag == "--instruction-limit") {
                try {
        ...
        if (target == "linux-x86_64" || target == "linux-x86-64") {
            const auto result = arco::fission::build_linux_native_image_file(argv[2], entry_function, output_path, native_debug_options);
            if (!result.ok) {
