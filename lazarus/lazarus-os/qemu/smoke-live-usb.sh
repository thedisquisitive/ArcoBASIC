...
usb_image="$work/live-usb-smoke.img"
...
qemu-system-x86_64 \
	...
	-device qemu-xhci \
	-drive file="$usb_image",format=raw,if=none,id=lazarus_live_usb \
...
for _ in $(seq 1 120); do
	if grep -q 'lazarus-xinitrc: exec Lazarus client' "$serial_log" 2>/dev/null &&
	   grep -q 'lazarus-service: socket ready' "$serial_log" 2>/dev/null &&
	   grep -q 'lazarus-network: Arcology Lazarus DHCP lease:' "$serial_log" 2>/dev/null &&
	   grep -q 'lazarus-printers: driverless discovery ready' "$serial_log" 2>/dev/null &&
	   ...
	   grep -q 'LAZARUS_INSTALL_TEST' "$serial_log" 2>/dev/null; then
		...
		grep -Eq 'lazarus-live: Writable live appliance state mounted from /dev/sd.*4 via ' "$serial_log"
