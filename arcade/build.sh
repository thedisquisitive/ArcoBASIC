#!/usr/bin/env bash
# Builds ARCADE's own capsule into arcade/build/ -- run from the repository root or anywhere else,
# it locates itself. Native always builds; the web capsule only builds if
# ARCOFISSION_WEB_TOOLCHAIN_DIR is set (see "Web Capsules" in docs/arcofission.md for how to set
# one up) so a plain desktop checkout doesn't need Emscripten installed just to build ARCADE.
#
...
echo "==> native: arcade/build/arcade"
"$ARCOFISSION" native "$SCRIPT_DIR/arcade.abas" -o "$SCRIPT_DIR/build/arcade"

if [ -n "${ARCOFISSION_WEB_TOOLCHAIN_DIR:-}" ]; then
    echo "==> web: arcade/build/web/arcade.html"
    "$ARCOFISSION" native "$SCRIPT_DIR/arcade.abas" -o "$SCRIPT_DIR/build/web/arcade.html" --target web
else
