...
its container. That capability now has its own RFC, **RFC-0053**
(`arcology-os/rfcs/RFC-0053_Native_Hosted_ArcoBASIC_Compilation_and_System_Runtime.md`), which is
...
**RFC-0053's** implementation, not ArcoSH's own session/RPM/protocol work. ArcoSH's own separate
...
## Entry 7 — 2026-09-05 — Reframed as general Arcology infrastructure; RFC-0053 filed
...
**What changed:**
- Filed **RFC-0053** (`arcology-os/rfcs/RFC-0053_Native_Hosted_ArcoBASIC_Compilation_and_System_Runtime.md`)
...
- Renamed both documents that had ArcoSH-specific names despite Entries 2-6 having nothing to do with
  ...
  - `.agents/ARCO_SH_PROGRESS.md` → `.agents/ARCO_NATIVE_RUNTIME_PROGRESS.md` (this file). A header note
    (top of file) explains the rename and points to RFC-0053; the original history below it is preserved
...
- Added a note to `.agents/reports/ARCO_SH_WP000_REPOSITORY_AUDIT.md` (ArcoSH's own, genuinely-ArcoSH-scoped
  WP-000 audit, left otherwise untouched) marking OQ-2 resolved and pointing to RFC-0053, so a future agent
  picking up ArcoSH's own WP-001 onward starts from "build against RFC-0053" rather than re-deriving this
  whole detour.
...
is also still open and is now more clearly ArcoSH's own question to resolve later, separate from RFC-0053.

**Next safe task:** Same as Entry 6's -- general function calls (fast-path-only slice) vs. Phase 2
(arrays/objects via ArcoValue) is still the live fork for RFC-0053's own next step. ArcoSH's own WP-001
remains not started, now clearly scoped as "build a session/RPM/protocol layer on top of RFC-0053 (or the
...
**Next safe task:** Phase 2 (arrays/objects) is the largest remaining item RFC-0053 lists as unattempted.
...
phase 2." Picked up RFC-0053's own remaining scope: arrays, objects, and classes via `ArcoValue`.
...
- Classes/methods (SELF, instance dispatch, EXTENDS/inheritance) -- the last major Phase 2 item
  RFC-0053 lists, not attempted this pass. Substantially bigger than arrays/objects alone: needs
...
**Next safe task:** Classes/methods is the largest remaining RFC-0053 item. A smaller, more
...
largest remaining RFC-0053 Phase 2 piece) and chained indexed assignment (`a.b.c = x`, Entry 9) are
...
next." Completes RFC-0053 Phase 2's own explicitly-carried-forward final item ("Classes and
...
**Next safe task:** With arrays, objects, and classes/methods all now real, RFC-0053's own
...
programs, ArcoSH included. The two RFC-0053-scoped items that were ALWAYS explicitly out of THIS
...
project this whole RFC-0053 effort was originally motivated by) can very plausibly now be started
...
testing (not just re-reading RFC-0053's own gap list) and presented a ranked set of real remaining
...
RFC-0053_Native_Hosted_ArcoBASIC_Compilation_and_System_Runtime.md` (Phase 6, and correcting two
...
language feature change well outside RFC-0053's own scope (native compilation), not attempted here.
...
## Entry 25 — Porting onto master for RFC-0052 (ArcoSH): a real, unresolved, BLOCKING loop bug found

**Date:** 2026-09-16

**Context:** this backend (through Entry 24, `7d44fbd`) was hand-isolated from the rest of this
branch's work (old-arcosh removal, `arcfs-utils/`, ArcoUI, the JIT compiler -- none of that was
wanted) and ported onto `master`, specifically to give RFC-0052's new ArcoSH implementation
(`arcosh/src/arcosh.abas`, master-only, does not exist on this branch) a real "compile to a native
executable" option instead of the bytecode-VM capsule format -- this backend's entire stated
motivation per RFC-0053 Section 1. Full regression suite green after the port (120/121, the one
failure a pre-existing unrelated `arcfsctl_smoke` PATH bug), including this file's own smoke test.
Two bugs found and worked around while getting `arcosh.abas` itself to compile; a third stopped the
effort from going further.

**Bug A (worked around, not a backend fix): `== TRUE`/`== FALSE` against a boolean is
unsupported.** `IF SomeCall() == FALSE THEN` (or any boolean value, not just a call result,
compared against a `TRUE`/`FALSE` literal) fails to compile with "unsupported operation '=='
on this backend" -- booleans apparently aren't a recognized operand kind for `==`'s own
classification the way numbers and strings are. Trivially avoidable: use the boolean directly
(`IF !SomeCall() THEN`) instead of comparing it to a literal. Not investigated further since the
workaround is strictly better ArcoBASIC style anyway (and this project already has its own
unrelated `NOT`-is-bitwise footgun to avoid, so `!x`/plain `x` over `x == FALSE`/`x == TRUE` was
already the right call).

**Bug B (worked around, not a backend fix): a `bool` returned DIRECTLY from a host function
crashes if fed straight into `ANDALSO`/`ORELSE`.** `is_tty = Console.IsTTY() : x = is_tty ANDALSO
TRUE` segfaults; confirmed NOT specific to any one function -- `File.Exists(...)` (a host function
that predates this backend entirely) reproduces the identical crash the same way. A bare `IF
is_tty THEN` / `IF !is_tty THEN` on the exact same host-returned value works fine; only the
short-circuit combination is affected. Comparison-expression results (`term <> "dumb"`,
`String.Slice(...) == ":"`) combine via `ANDALSO`/`ORELSE` without issue -- this is specific to a
raw boxed-bool value coming back through the generic host-function bridge
(`src/native/host_bridge.cpp`) into a short-circuit operator, not `ANDALSO`/`ORELSE` in general.
Worked around in `arcosh.abas` with nested `IF` instead of chaining the host-returned bool through
`ANDALSO`. Root cause not investigated -- likely the bridge's bool-boxing leaves some type/tag
state the short-circuit codegen's fast path doesn't expect, but this is a guess, not a finding.

**Bug C (NOT worked around -- real, reproducible, BLOCKING; native codegen NOT switched on for
arcosh as a result): a loop body with two or more host-function calls per iteration corrupts
whichever host call happens first on the NEXT iteration.** Minimal repro (isolated well below
arcosh.abas's own size):
...
Piped one line of stdin then EOF (`printf "pwd\n" | ./this_native_binary`): the FIRST loop
iteration works correctly (reads the line, prints the cwd). The SECOND iteration's
`Console.ReadLine()` -- which should see EOF and return `Ok=FALSE` -- instead returns `Ok=TRUE`
with an empty string, forever, spinning at 100% CPU printing `Path.Cwd()` in an infinite loop
rather than exiting. Confirmed NOT a `CONTINUE WHILE`-specific issue (a variant with only one
`CONTINUE WHILE`, EOF-only, still hangs the instant a second host call -- here `Path.Cwd()` -- is
added to the loop body) and NOT a language/interpreter bug (`ArcoFission compile-run` on the
identical source prints "clean exit" correctly, every time). This is specific to this backend's
loop-body codegen when multiple host-function calls happen per iteration -- exactly the shape any
real interactive shell REPL needs (read input, then dispatch to at least one more host call every
single iteration), so this blocks using this backend for `arcosh`'s actual interactive loop today.

**Status: open, not fixed.** Not investigated further this entry -- diagnosing this properly needs
the same discipline every other bug in this file was fixed with (ASan build, `gdb`/disassembly on
the actual generated loop-body machine code, understanding exactly what the second host call's own
calling-convention setup clobbers that the next iteration's first call depends on) rather than a
guess-and-check fix from someone unfamiliar with this codegen's register allocation. `arcosh/`
stays on the bytecode-VM capsule build (`ArcoFission build ... ` with no `--target`) for its actual
shell loop until this is root-caused; the native backend itself, and its ability to compile
`arcosh.abas`'s pure-computation Path.* layer standalone (proven: all 16 `--selftest-paths`
assertions pass identically under `--target linux-x86_64`), remain real and available for future
work once Bug C is fixed.

## Entry 26 — Bug C root-caused and fixed: `!` on a Boxed operand read raw pointer bits, not the value

**Date:** 2026-09-17

**Root cause.** `generate_x86_64_function`'s Unary case already had the right idea for `-`/`~`:
check `infer_hosted_value_kind` and route a genuinely Boxed operand through a proper unboxing call
(`load_double_operand` -> `arco_value_as_number`) rather than operating on raw register bits. `!`
had no equivalent check at all -- it always fell through to the generic path, which loads the
operand into RAX with the FRONTEND's own (often-empty/wrong-for-a-dynamic-value) type hint and does
`cmp rax, 0` / `sete` / `movzx` directly on whatever's there. For a Boxed operand (e.g. `line.Ok`,
the result of `arco_value_object_get`, which -- confirmed by reading `runtime_abi.cpp` directly --
ALWAYS heap-allocates a fresh `ArcoValueBox` regardless of the field's actual type) that "whatever's
there" is a real heap pointer, and `(pointer & 0xff) == 0` (the actual generated sequence includes
an `and rax, 0xff` from the frontend's own "BOOL" type hint before the compare) depends entirely on
the allocator's own address for that specific call site under that specific allocation history --
NOT on the value inside the box. This is why it looked "sometimes correct": a fixed, repeating
allocation/free cycle (no other intervening allocations) returns the identical address every time
regardless of what's stored there, so the check's WRONG answer stayed constant and looked plausible
in the smallest single-host-call repros; adding literally any second host call anywhere in the same
loop shifted the free-list state enough to flip that constant to the OTHER wrong-but-stable answer,
manifesting as EOF (or whatever the false case was) never being detected again for the rest of the
run. Confirmed AddressSanitizer-clean throughout (this is a logic bug, not a memory-safety one) --
matches Entry 20's own "ambiguous physical representation across code paths" pattern, one call
family over: `arco_value_object_get`'s always-boxed return vs. `!`'s assumption it might already be
a raw unboxed 0/1, same shape as `load_double_operand`'s own doc comment already warned about for
`+`/`-`/`*`/`/` ("the same failure class as the earlier BOOL-as-double bug") -- `!` was simply the
one arithmetic/logical operator that check was never extended to.

**Fix.** Added `arco_value_truthy` to the native ABI (`native_runtime_abi.h`/`runtime_abi.cpp`),
mirroring `arco_value_as_number`'s existing shape exactly (never panics, delegates to
`arco::Value::truthy()`, null-pointer-safe). `generate_x86_64_function`'s Unary case now checks
`infer_hosted_value_kind(...) == HostedValueKind::Boxed` for `!`'s own operand (the identical
condition `-`/`~` already use, just applied to a case that had no such check before) and calls
`arco_value_truthy` through the standard single-pointer-argument external-call sequence before the
same `cmp`/`sete`/`movzx` negation, instead of operating on the raw loaded value at all. The
existing raw-bit path is kept, unchanged, for genuinely non-Boxed operands (a plain comparison
result, an already-unboxed local) -- this is strictly additive, not a rewrite of a working path.

**Verification.** All of Bug C's own repros (the two-call `Console.ReadLine`+`Path.Cwd` WHILE loop,
`Console.IsTTY()` x2 + `ReadLine`, and the further-minimized `ReadLine` + one bare extra host call)
now terminate correctly on EOF instead of spinning. The full `linux_native_backend_smoke` suite
(the project's own most comprehensive native-backend regression coverage, including compiling and
live-driving Arconaut) passes unchanged (351s, 0 failures) -- this fix touches a path that test
suite apparently never happened to exercise with a Boxed `!` operand specifically. Full project-wide
`ctest` otherwise unaffected (121/122; the one failure is the pre-existing, unrelated
`arcfsctl_smoke` PATH bug). `arcosh/src/arcosh.abas` (RFC-0052) now compiles and runs correctly
under `--target linux-x86_64` with its real interactive REPL loop intact --
`arcosh/CMakeLists.txt` switched over to the native build as a direct result. Bugs A (`==
TRUE`/`FALSE` against a boolean is unsupported) and B (a host-returned bool crashes if fed directly
into `ANDALSO`/`ORELSE`) from Entry 25 are a different code path (Binary/comparison codegen, not
Unary) and remain open, unaffected by this fix -- `arcosh.abas` still avoids both by ArcoBASIC-level
style (`!x` over `x == FALSE`, nested `IF` over chaining a host-returned bool through
`ANDALSO`/`ORELSE`), which was already the right call independent of this fix.

## Entry 27 — A second real miscompilation: `LEN(text)` on a hosted string silently returned garbage

**Date:** 2026-09-17

**Context:** implementing ArcoSH WP-003 (structured command parser) -- a tokenizer that walks a
command string character-by-character in a `WHILE i < length` loop, `length = LEN(text)`. Worked
perfectly under `compile-run` (the interpreter); under `--target linux-x86_64` the loop always
stopped after exactly 1 iteration, no matter the input length, no crash, no ASan hit.

**Minimized repro** (well below the tokenizer's own size):

```basic
FUNCTION CountWithArgs(text AS STRING)
    count = 0 : i = 0 : length = LEN(text)
    WHILE i < length
    ...
    RETURN count
END FUNCTION
PRINT STRING(CountWithArgs("ls-la"))   ' prints 1, not 5
```

Bisection found the trigger precisely: `length = LEN(text)` (not a literal) PLUS any host-function
call anywhere in the loop body (even a zero-argument one like `Console.IsTTY()`) together produced
the wrong count; either alone was fine. Adding a debug `PRINT` of `length` inside the loop showed
its stored value as a denormal double like `1.4822e-323` -- a small integer's raw bit pattern
reinterpreted as a double, the same signature as every other bug in this file caused by treating
one representation as another.

**Root cause.** `generate_x86_64_function`'s `Kind::CallValue` case has TWO different `LEN`/`MID`
code paths: an old one (predating the System V/hosted backend entirely) that hand-walks a raw,
null-terminated UTF-16 buffer pointer -- correct ONLY for the freestanding target's own string
representation -- gated by `!instruction.operand_types.empty() && instruction.operand_types.front()
== "STRING"`; and a newer, correct one further down that calls the real `arco_value_length` ABI
function, gated on `infer_hosted_value_kind(...) == Boxed/String`. The first gate never checked
`convention` at all. `lower_call` sets that "STRING" operand-type hint for LEN/MID whenever the
argument is textually typed STRING, regardless of which backend is compiling -- so an entirely
ordinary hosted `FUNCTION f(text AS STRING) ... LEN(text)` under System V fell into the
freestanding-only raw-buffer walk, which treated `text`'s real Boxed `ArcoValue*` pointer as a
UTF-16 buffer and walked off into unrelated heap memory until it happened to find a zero unit --
producing a small, wrong, but perfectly plausible-looking `U64` length that then quietly governed
every loop bound built from it. This is why literal-length loops (`length = 5`) were unaffected --
they never call `LEN` at all -- and why a second host call was necessary to reproduce: it wasn't
the trigger, `LEN`'s own miscompilation was; the second call just added enough register/stack
pressure to reveal the wrong value making the loop misbehave concretely enough to notice as "stops
after 1 iteration" rather than some other-shaped wrong answer.

**Fix.** Added `convention == systems::CallingConvention::MicrosoftX64 &&` to that first gate's own
condition (`src/compiler/fission.cpp`, the `string_len_or_mid` local). The freestanding raw-buffer
walk now only ever fires for the UEFI target it was written for; every System V `LEN`/`MID` call
falls through to the already-correct, already-existing Boxed-aware path below it, unchanged.

**Verification.** All bisection repros (`length = LEN(text)` + a no-arg call, + a `String.Slice`
call, the full tokenizer) now report the correct count/output. Full `linux_native_backend_smoke`
suite unaffected (357s, 0 failures) -- this exact LEN(hosted-string-parameter)-in-a-loop shape
apparently never came up in it. Full project-wide `ctest` otherwise unaffected (119/120, the one
failure the pre-existing unrelated `arcfsctl_smoke` bug). `arcosh.abas`'s new WP-003
`TokenizeCommand`/`ParseCommand` and WP-004 `Process.Execute`-backed command execution (real
`ls`/`git status`/quoted-argument runs, clean command-not-found handling, and -- verified under a
real pty -- correct job-control terminal handoff where Ctrl-C during a foreground child interrupts
the child and returns control to a still-responsive shell) all work correctly compiled natively as
a direct result.

## Entry 28 — A confirmed language-level gap (not backend-specific): string relational comparison, plus a real parameter-typing requirement for numeric fields pulled out of a dynamically-typed object

**Date:** 2026-09-17

**Context:** implementing ArcoSH WP-006 (resident numbered-program editor, RFC-0052 section 8/9).
`ParseLeadingLineNumber` needed digit detection while scanning a line character-by-character; the
natural expression is `ch >= "0" ANDALSO ch <= "9"`.

**Finding 1 (language gap, not this backend).** `ch >= "0"` throws "value is not a number" under
`compile-run` (the interpreter itself) and fails native compilation with "unsupported operation
'>=' on this backend" -- string relational comparison (`>=`/`<=`) is simply not implemented
anywhere in the language, interpreter included. This is not a codegen bug to fix in `fission.cpp`;
it would need frontend/interpreter support that doesn't exist. Worked around directly:
`String.Contains("0123456789", ch)` for digit detection, avoiding the operator entirely.

**Finding 2 (real, if narrow, native-backend requirement).** `SetProgramLine(program AS ARRAY,
lineNumber, text AS STRING)` -- an untyped `lineNumber` parameter -- failed native compilation at
every call site with: `call to "SetProgramLine" passes a array/object argument to parameter
"lineNumber", which has no explicit type annotation`, even though every call site passes
`someParsedResult.Number`, a genuine number field off an object literal built with `NUMBER(digits)`
(see `ParseLeadingLineNumber`). The native backend's argument-classification pass evidently can't
statically see through a field access on a dynamically-typed (Boxed) object to know the field's
runtime type is numeric, so it falls back to treating the call as passing a non-numeric value and
requires the same explicit annotation non-numeric parameters already need (`AS ARRAY`/`AS
STRING`/`AS OBJECT`) -- just for the numeric case too (`AS NUMBER`). Fixed by annotating the
parameter `lineNumber AS NUMBER`; no `fission.cpp` change needed since this is (confirmed by
reading the diagnostic message and the annotation actually working) already-correct, intentional
gating behavior once you know it exists -- documented here because it wasn't obvious in advance
and cost real time to isolate from Finding 1's compile error, which looked superficially similar.

**Verification.** `arcosh --selftest-resident` passes all eight assertions (insert/replace/
delete/reorder plus a real `Runtime.RunString`-executed `GOTO` loop) on the native binary; manual
interactive verification against the RFC section 9 worked example (`10 PRINT "ONE"` / `20 PRINT
"TWO"` / replace `20 PRINT "THREE"` / `LIST` / bare `20` deletes it / `LIST` / `RUN` / `NEW`) and a
separate multi-digit-line-number `GOTO` counting loop both matched expected output exactly. Full
`arcosh/build` ctest suite: 5/5.

## Entry 29 — Fixing Entry 28's Finding 1 for real: string relational comparison, in all FOUR places it was independently missing

**Date:** 2026-09-17

**Context:** Entry 28 documented `>=`/`<=` on strings as a language-level gap and worked around it
in `arcosh.abas`. Asked directly afterward whether it should just be fixed instead of worked
around -- yes, and it turned out to be missing independently in four separate places, not one:

1. The tree-walking interpreter's `apply_binary` (`src/frontend/parser.cpp`) -- `Less`/`LessEqual`/
   `Greater`/`GreaterEqual` all called `a.as_number() OP b.as_number()` unconditionally.
2. The bytecode VM's `eval_binary` (`src/compiler/fission.cpp`) -- an entirely separate,
   textually near-identical copy of the same four cases (this is what `compile-run` actually
   executes -- it runs the bytecode capsule, not the tree-walker directly, despite the tree-walker
   also existing and sharing the bug).
3. The native backend's ambiguously-Boxed/String comparison block (`generate_x86_64_function`,
   the one already widened for "==" / "!=" in the RFC-0053 "full Linux support" pass) -- only
   handled equality, fell through relational operators to the ordinary numeric `ucomisd` path.
4. The native backend's provably-STRING fallback block (same function, further down) -- the one
   that actually produced the exact error text this project's own `ParseLeadingLineNumber` hit
   ("unsupported operation '>=' on this backend"): handled `+` (concat) and `==`/`!=`, then fell
   straight to its own "unsupported operation" error for anything else.

**Fix.** Added one shared ordering primitive, `arco::values_compare(left, right)` -> -1/0/1
(`include/arco/value.hpp`, right next to the pre-existing `values_equal`): stringifies and compares
lexicographically if either operand is a string (mirroring `apply_binary`'s own Plus case -- "either
side a string wins"), otherwise coerces both to numbers via `as_number()` (still throws for a
genuinely non-orderable value -- arrays/objects/handles never supported ordering and still don't).
Both interpreter call sites (#1 and #2 above) now just call `values_compare(...) < 0` / `<= 0` /
`> 0` / `>= 0` instead of duplicating the comparison logic. Block #3 was widened to include all
four relational operators alongside "=="/"!=" (its own guard already covered Boxed-or-String on
either side, so no new guard was needed, just more operators recognized inside it) -- which turned
out to make block #4 unreachable for these operators too, the same way the "==" / "!=" widening in
the earlier pass made block #4 unreachable for those (left in place as a harmless fallback, per
that pass's own reasoning, rather than deleted). A new ABI function, `arco_value_compare`
(`include/arco/native_runtime_abi.h` / `src/native/runtime_abi.cpp`), mirrors `values_compare`
exactly for the native call sites, the same way `arco_value_equals` already mirrors `values_equal`.

**A second, independent bug found WHILE wiring up the ABI call.** The generated code for block #3
turns `arco_value_compare`'s -1/0/1 result into a BOOL with `cmp rax, 0` + `setcc` -- but the ABI
function returns a 32-bit `int`, and the System V calling convention leaves the upper 32 bits of
RAX **unspecified** for a 32-bit return value (never guaranteed sign-extended by the callee). A
64-bit `cmp rax, 0` against the -1 case was comparing whatever garbage happened to be sitting in
RAX's high bits, not really -1 -- confirmed directly: `"abc" < "abd"` (a genuine "compare < 0"
case) came back `FALSE` with the ABI call and widened operator list both correctly in place, while
`"abc" <= "abc"` (the `== 0` case, high bits irrelevant since low 32 bits alone already read as
zero either way... except cmp is 64-bit, so even THIS should have been at risk, and mostly wasn't
just because this toolchain's calling convention happens to usually leave zero up there for a
small non-negative return -- not a guarantee, just this run's luck) and `"b" > "a"` (`> 0`) both
happened to read correctly by the same luck. Fixed by sign-extending EAX into RAX first (`shl rax,
32` / `sar rax, 32` -- a CDQE-equivalent built from two existing shift-by-immediate encoder methods,
since this encoder has no dedicated CDQE opcode) before the 64-bit `cmp`.

**Verification.** A dedicated repro script (string `<`/`<=`/`>`/`>=` literals, `ANDALSO`-combined
digit-range checks matching `ParseLeadingLineNumber`'s exact shape, and pure-numeric comparisons
as a regression check) produces IDENTICAL output on `compile-run` (bytecode VM) and
`--target linux-x86_64` (native): `TRUE TRUE TRUE TRUE TRUE TRUE TRUE FALSE` for the first script,
matching hand-computed expected values for all eight cases including the previously-wrong `<=` and
`<` results. Full project rebuild (`cmake --build build`) clean; full root `ctest` suite unaffected
at its established baseline; `arcosh`'s own full ctest suite (5/5, including
`--selftest-resident`, which exercises `ParseLeadingLineNumber`'s digit-range check on the native
binary via its `String.Contains` workaround, now provably redundant but left in place since it
already works and changing working code purely for its own sake isn't in scope here).

## Entry 30 — A third real miscompilation: `LOAD` masked a genuinely Boxed local down to its low byte when the frontend's own per-branch type hint said "BOOL"

**Date:** 2026-09-17

**Context:** implementing ArcoSH WP-011 (pipelines and redirection, RFC-0052 AP-0052-014).
`ExecutePipelineAndRecord`'s stdout-redirection handling declares `appendStdout = FALSE` (a plain
literal) before an `IF`, then conditionally reassigns it from `stdoutInfo.Append` (a Boxed
object-field read off `FindStdoutRedirection`'s returned object) inside it -- classic BASIC "start
with a default, override under a condition" shape. `STRING(appendStdout)` segfaulted immediately
after, regardless of which branch actually ran.

**Minimized repro:**

```basic
FUNCTION Outer(stdoutInfo AS OBJECT)
    appendStdout = FALSE
    IF stdoutInfo.Target <> "" THEN
        appendStdout = stdoutInfo.Append
    END IF
    s = STRING(appendStdout)   ' segfault, either branch
END FUNCTION
Outer({"Target": "", "Append": TRUE})
```

**Root cause.** This project's OWN existing machinery already handles exactly this "a local
reassigned with two genuinely different representations across branches" shape correctly in
principle: `infer_local_kind` (`src/compiler/fission.cpp`) scans every `Store` targeting a local
and, when their inferred kinds disagree (here: a literal `FALSE`, classified `Bool`, vs a Boxed
object-field read, classified `Boxed`), resolves the local's overall static answer to `Boxed` --
and `Kind::Store`'s own codegen already boxes the literal `FALSE` via `arco_value_new_bool` in that
case specifically so the answer holds no matter which branch runs (this exact mechanism, and the
`quoted = quoted + ch`/Arconaut ShellQuote bug that motivated it, predates this session). Confirmed
via `--debug` assembly that this part worked exactly as designed: the literal store really does
call `arco_value_new_bool` first.

The bug was one level up, in `Kind::Load`. A prior fix (RFC-0053, "full Linux support" pass,
predating this session) already taught `Kind::Load` to distrust the frontend's own static
`instruction.result_type` hint and prefer `infer_hosted_value_kind`'s real, disagreement-aware
answer -- but ONLY for the `Number` case (`x = (i < n) AND (count < 3)` printing "0" instead of
"1", documented in the surrounding comment). It never checked for `Boxed`. So loading `appendStdout`
back out still fell through to the frontend's own naive per-statement hint -- "BOOL", since
syntactically `appendStdout = FALSE` looks like a plain bool assignment with no visibility into the
OTHER branch's Boxed reassignment -- and normalized the loaded value with an `AND 0xFF` byte mask,
the ordinary BOOL-narrowing operation. Confirmed directly in the emitted assembly (`--debug`):
`LOAD appendStdout` executed `mov rax, [slot]; and rax, 0xFF; and rax, 0xFF` (an actual DOUBLE mask,
redundant AND included) before handing that truncated 0/1 "pointer" to `STRING()`'s generic
host-call argument marshaling, which dereferenced it as a real `ArcoValue*` -- a guaranteed
segfault for any address other than a lucky small value.

**Fix.** Widened the exact same preference `Kind::Load` already applies for `Number` to also cover
`Boxed`: `load_result_type` is now `"U64"` (no masking) whenever `infer_hosted_value_kind` answers
either `Number` or `Boxed`, falling back to the frontend's own hint only when the real answer is
something else (`Unknown`, or a case this analysis genuinely can't resolve). One boolean-condition
change plus a `HostedValueKind load_result_kind` local to avoid computing it twice.

**Verification.** Both minimized repros (branch taken and not taken) now print the correct
`FALSE`/`TRUE` instead of segfaulting. `ExecutePipelineAndRecord`'s own full shape (built as a
standalone repro before touching `arcosh.abas` itself) runs correctly end-to-end. `arcosh`'s full
`--selftest-pipeline` (6 assertions: a real `cat | grep -c` pipeline, plain `>` redirection, `>>`
append verified by real line count, combined `<`+`>` in one command, a pipeline surviving an
earlier stage's command-not-found, and per-command history-entry counting -- all against REAL
