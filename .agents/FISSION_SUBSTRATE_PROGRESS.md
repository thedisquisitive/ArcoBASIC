...
printed the correct `6\n6`. See Completed Components for the full writeup,
including the real binary-vs-text bytecode format distinction this
uncovered, and a genuine Rivet caching bug found and fixed along the way.
...
D. DONE for the bytecode-VM-embedded capsule shape -- see Completed
   Components. `ArcoFission package FILE.arcof -o OUT` packages an existing
   `.arcof-text` bytecode file (including Fission substrate's own output)
   into a real standalone Linux ELF64 capsule, reusing legacy ArcoFission's
   existing bytecode-embedding launcher machinery entirely -- zero new ELF64
   writing needed after all. Verified: the CLASS smoke test's
   substrate-generated bytecode, packaged this way, runs correctly as a bare
   binary with no `ArcoFission` CLI involved. What's NOT done: a Rivet
   `ArcoCapsuleTarget`-equivalent build-graph target that drives `package`
   for a Fission-substrate-compiled program the way `ArcoCapsuleTarget`
   already does for legacy ArcoBASIC source (real but small follow-on --
   mirror the existing `ArcoCapsuleTarget.Build()` shape in
   `rivet/stdlib/rivet.abas`, pointed at `package` instead of `build`/
   `native`); and a real ELF64 *writer* genuinely still does not exist
   anywhere in this project -- everything above works by reusing legacy
   ArcoFission's C++ linker invocation, not by this substrate emitting ELF64
   bytes itself. If a target ever needs an ELF64 capsule with zero legacy
   C++ compiler involvement at build time (e.g. for a future self-hosting
   milestone), that remains real, unstarted, sizeable work.
